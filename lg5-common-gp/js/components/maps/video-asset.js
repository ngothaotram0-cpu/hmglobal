...
$(document).ready(function () {
  ...
  var closeTxt =
    $(".navigation").length > 0 && $(".navigation").data("close-video")
  ...
  video = {
    ...
    markup: {
      modal: [
        ...
        '</div></div><button class="close-video">' +
          closeTxt +
          "</button></div>",
      ],
      //LGEPE-323 Start : modal_mobile Add
      modal_mobile: [
    ...
    trigger: {
      ...
      setYoutube: function (t) {
        ...
        //LGEPE-323 Start
        var scaMobileFlag = false;
        var videoViewType = xssfilter(t.getAttribute("data-view-type"));
        if ($(".GPC0074 .slick-list").length > 0) {
          if (true == isMobile) {
            videoId = xssfilter(t.getAttribute("data-mobile-src"));
            scaMobileFlag = true;
          }
        }
        //LGEPE-323 End
        ...
        var playsinline =
          t.getAttribute("data-target") != "modal" ? "&playsinline=1" : "";
        htmlDefault = '<div id="videoPlayerCode"></div>';
        if (t.getAttribute("data-target") == "modal") {
          //LGEPE-323 Start
          if (scaMobileFlag == true && "vertical" == videoViewType) {
            html =
          ...
          html =
            video.markup.current[0] + htmlDefault + video.markup.current[1];
      ...
      setBrightcove: function (t) {
        ...
        if ($(".GPC0074 .slick-list").length > 0) {
          if (true == isMobile) {
            videoId = xssfilter(t.getAttribute("data-mobile-video-id"));
            scaMobileFlag = true;
          }
        }
        //LGEPE-323 End
          ...
          function onPlayerStateChange(event) {
            if (event.data == YT.PlayerState.PLAYING) {
              adobeSatellite("video_track", {
                name: video.getTitle("youtube", player),
                event: "play",
              });
            }
        ...
        setTimeout(function () {
          ...
          function callback() {
            ...
            myPlayer.on("pause", function () {
              adobeSatellite("video_track", {
                name: video.getTitle("brightcove", myPlayer),
                event: "pause",
              });
            });
            ...
            "click.video": function (e) {
              ...
              if ($("#videoPlayerCode").length > 0) {
                ...
                if (oldVideo.closest(".video-modal").length > 0) {
                  oldVideo
                    .closest(".video-modal")
                    .find("button.close-video")
                    .trigger("click");
                } else {
                  $("#videoPlayerCode")
                    .siblings("button.close-video")
                    .trigger("click");
                }
              }
              ...
              if (_this.getAttribute("data-type") == "youtube") {
                ...
                if (
                  ...
                  if (_this.getAttribute("data-target") != "modal") {
                    ...
                    if ($cv) {
                      ...
                      if (typeof video.playVideoYoutube == "function")
                        video.playVideoYoutube($ci);
                      unCache = false;
                    } else {
                      unCache = true;
                    }
                ...
                if (_this.getAttribute("data-target") != "modal") {
                  ...
                  if ($cv) {
                    ...
                    if (typeof video.playVideoBrightCove == "function")
                      video.playVideoBrightCove($ca, $cp, $ci);
                    unCache = false;
                  } else {
                    unCache = true;
                  }
    ...
    appendCurrent: function (
      ...
      $target = $target.get(0)
        ? $target
        : $(anchor).parents(".video-box").find(".visual-area");
      ...
      $(".video-asset").on(
        {
          click: function (e) {
            ...
            var $invisible = $(e.delegateTarget)
              .parents(".video-box")
              .find(".text-area");
