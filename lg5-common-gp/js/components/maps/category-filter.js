...
var productType =
  $("#resultProductList .cardlist-box").length > 0 ? "card" : "default";

// PJTQUICKWIN - 20220225
var cheangStatusModelGroup = function ($obj) {
  var _this = $obj;
  var obj_ea = _this.find("a").length;
  var group_width = _this.width();
  ...
  if (productType == "card") {
    ...
    _this.find(".under2").removeClass("under2");

    if (obj_ea * obj_width > group_width) {
      _this.addClass("limited");

      var countItemViewport = 0;
      var firstItemOffsetTop = _this.find("a").eq(0).offset().top;
      for (var i = 0; i < obj_ea; i++) {
        if (_this.find("a").eq(i).offset().top == firstItemOffsetTop) {
          countItemViewport++;
        }
      }

      if (obj_ea - countItemViewport == 0) {
        _this.removeClass("limited");
        _this.find(".btn-limited").remove();
      }

      for (var j = 0; j < obj_ea; j++) {
        if (j > countItemViewport - 1) {
          _this.find("a").eq(j).addClass("under2");
        }
      }

      if (!_this.find("button").hasClass("btn-limited")) {
        _this.append(
          '<button class="btn-limited">' +
            "+ " +
            (obj_ea - countItemViewport) +
            "</button>"
        );
        ...
        _this.find(".btn-limited").text("+" + (obj_ea - countItemViewport));
      }
    } else {
      _this.removeClass("limited");
      _this.find(".btn-limited").remove();
    ...
    return; // 카드 타입인 경우 아래 default 코드 실행 안됨
  }
  ...
  if (obj_ea * obj_width + obj_etc > group_width) {
    ...
    if (!_this.find("button").hasClass("btn-limited")) {
      _this.append(
        '<button class="btn-limited">' + "(" + obj_ea + ")" + "</button>"
      );
      _this.find("button").on("click", function (e) {
        if ($(window).width() < 768) {
          _this.toggleClass("open");
...
function initWhiteSpace() {
  (function () {
      ...
      setTimeout(function () {
        if (winW < 769 && $(".product-list-box").hasClass("rows")) {
          ...
          $.each(_this, function (i) {
            if (
              _this.eq(i).find(".price-area").css("height") === "0px" &&
              _this.eq(i).find(".price-vip-Installment").css("height") ===
                "0px" &&
              _this.eq(i).find(".stock-area").css("height") === "0px"
            ) {
              $(this).hide();
            } else {
              if (
                _this.eq(i).find(".promotion-text").length == 0 &&
                _this.eq(i).find(".promotion-text").css("height") === "0px"
              ) {
                $(this).hide();
              } else {
                $(this).show();
    ...
    $("[data-category-id]").each(function () {
      ...
      //PJTLIMITQTY_EXTEND
      var selector = '[categoryId="{{categoryId}}"]'.replace(
        "{{categoryId}}",
        categoryId
      );
      if (tagVal == "N") {
      ...
      if (priceVal == "N") {
        ...
        if (priceVal == "N" && promotionVal == "Y") {
          $(selector)
            .find("[data-white-type='priceAreaYn']")
            .find(".price-area")
            .remove();
          $(selector)
            .find("[data-white-type='priceAreaYn']")
      ...
      var $modelButtonEl = $(selector).find(
        ".products-info .model-button .button"
      );
      ...
      if (
        (emiVal == "Y" && emiUseFlag == "Y") ||
        (limitSaleAreaVal == "Y" && limitSaleUseFlag == "Y")
      ) {
        $(selector)
          .find("[data-white-type='priceAreaYn']")
          .addClass("has-topInfo");
      }
    });
  })();
...
$(document).ready(function () {
  ...
  var removeTxt = $(".apply-filters").attr("data-title-close")
    ? $(".apply-filters").attr("data-title-close")
    : "remove";
        ...
        if (va == "view-all") {
          ...
          if ($("#result-box-aria").length > 0) {
            cookieText =
              location.pathname +
              "=" +
              $(filter2.el).find("input[type=hidden][name=page]").val();
          }
          if ($("#categoryFilterForm").length > 0) {
            cookieText =
              location.pathname +
              "=" +
              $(filter.el).find("input[type=hidden][name=page]").val();
          }
        }
        if (
          ...
          COUNTRY_CODE.toLowerCase() == "es" ||
          COUNTRY_CODE.toLowerCase() == "fr"
        ) {
          //LGEDE-926,LGEIS-1124,LGEES-481,LGCOMFR-46
          ...
          if (cate == location.pathname) {
            if (page == "viewAll") {
              if ($("#categoryFilterForm").length > 0) {
                ...
                $(filter.el)
                  .find("input[type=hidden][name=page]")
                  .val("viewAll");
              }
              // Use for PJTCUR-3
              if ($("#result-box-aria").length > 0) {
  ...
  if (
    $("[name=bundlesOnly]").length > 0 &&
    $("[name=bundlesOnly]").parent().find(".filter-cnt").text() == "(0)"
  ) {
    $("[name=bundlesOnly]").prop("disabled", true);
  }
  if (
    $("[name=promotionsOnly]").length > 0 &&
    $("[name=promotionsOnly]").parent().find(".filter-cnt").text() == "(0)"
  ) {
    $("[name=promotionsOnly]").prop("disabled", true);
  }
  if (
    $("[name=obsOnly]").length > 0 &&
    $("[name=obsOnly]").parent().find(".filter-cnt").text() == "(0)"
  ) {
  ...
  if ($("#categoryFilterForm").length > 0) {
    ...
    filter = {
        ...
        init: function () {
          ...
          $(_this.close).on({
            click: function (e) {
              ...
              if ($(filter.el).is(":visible")) {
                ...
                /* 20191002 : LGEUS-12351 add */
                if ($(".GPC0007").length > 0) {
                  $(".GPC0007 .filter-open-floating").removeClass(
                    "no-floating"
                  );
                }
                /* //20191002 : LGEUS-12351 add */
                // LGEGMC-526
                if ($(".GPC0026").length > 0) {
                  $(".GPC0026 .filter-open-floating").removeClass(
                    "no-floating"
                  );
                }
                // LGEGMC-526 End
          ...
          if (
            ...
            if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
              var filterLine = parseInt(
                $(".GPC0007, .GPC0026").find(".product-list-box").offset().top
              );
              var filterTopLine = parseInt(
                $(".GPC0007, .GPC0026").offset().top
              );
              ...
              $(window).on("load scroll", function () {
                ...
                var scrollPos =
                  ...
                  parseInt(controlStickyElements.pos.FILTER);
                var isFilterFloating = false;
                if (
                  ($(".GPC0007 .filter-open-floating").length > 0 &&
                    !$(".GPC0007 .filter-open-floating")
                      .attr("class")
                      .match("no-floating")) ||
                  ($(".GPC0026 .filter-open-floating").length > 0 &&
                    !$(".GPC0026 .filter-open-floating")
                      .attr("class")
                      .match("no-floating"))
                ) {
                  isFilterFloating = true;
                }
                // PJTQUICKWN START - 박지영
                // 7번 컴포넌트와 151이 같이 있는 경우,,, filter가 floating 되는 위치가 달라집니다.
                // if($(".GPC0151").length > 0) {
                // 	filterLine = $('.GPC0151').offset().top * 1.2;
                // }
                // PJTQUICKWN END - 박지영
                if (scrollPos >= parseInt(filterLine) && isFilterFloating) {
                  if (
                    ...
                    $(".GPC0007, .GPC0026")
                      .find(".filter-open-floating")
                      .removeClass("float-fix")
                      .addClass("float-active");
                    if (
                      $(".eprivacy-cookie").hasClass("cookie-eu") &&
                      $(".cookie-banner").hasClass("ready") &&
                      ($("html").attr("data-countrycode") == "uk" ||
                        $("html").attr("data-countrycode") == "fr" ||
                        $("html").attr("data-countrycode") == "nl" ||
                        $("html").attr("data-countrycode") == "be_fr")
                    ) {
                  ...
                  if (
                    $(
                      ".GPC0007 #categoryFilterForm, .GPC0026 #categoryFilterForm"
                    )
                      .attr("class")
                      .match("open")
                  ) {
                    $(".GPC0007, .GPC0026")
                      .find(".filter-open-floating")
                      .addClass("no-floating");
                  } else {
                    $(".GPC0007, .GPC0026")
                      .find(".filter-open-floating")
                      .removeClass("float-active");
                  }
                }
              });
              $(".GPC0007 .filter-open-floating .floating-filter").on({
                click: function (e) {
                  ...
                  if ($(this).attr("class").match("active")) {
                    ...
                    $(".filter-box .float-filter-result").css(
                      "display",
                      "none"
                    );
                    $(".filter-box .filter-result").css("display", "block");
                    // floating-filter 클릭시 위치 조절
                    filterTopLine =
                      parseInt($(".GPC0007").offset().top) -
                      parseInt(controlStickyElements.pos.FILTER);
              ...
              $(
                ".GPC0007 .filter-open-floating .float-sort-box .sort-select .sortBy"
              ).on({
                change: function (e) {
                  if (
                    typeof ePrivacyCookies == "undefined" ||
                    ...
                    var sortTxt = changeTitleFormat(
                      $(this).find("option").eq(_this.selectedIndex).text()
                    );
                    plp.el.form.sort.value = sort;
                    plp.el.form.page.value = 1;
                    /* BTOBGLOBAL-434 : 20210408 add */
                    if (
                      COUNTRY_CODE.toLowerCase() == "global" &&
                    ...
                    $(plp.el.form).submit();
                    $(
                      ".GPC0007 .filter-open-floating .floating-filter"
                    ).removeClass("active");
                    $(".GPC0007 #categoryFilterForm").removeClass("open");
                    $(".GPC0007 .filter-open-box").css("display", "block");
                    $(".GPC0007 .filter-open-floating").removeClass(
                    ...
                    $(".filter-box .filter-result").css("display", "block");
                    $(".result-box .sort-box .sort-select")
                      .find('select option[value="' + sort + '"]')
                      .prop("selected", true);
                    $(".result-box .sort-box .sort-select")
                    ...
                    $("html, body").animate(
                      {
                        scrollTop: $(plp.el.scrollTarget)
                          .closest(".component")
                          .offset().top,
                      },
                      500
                    );
                    adobeTrackEvent("product-list-sort", {
                      sort_option: sortTxt,
                      page_event: { product_sort: true },
                    });
                  } else {
              ...
              // LGEGMC-526
              $(
                ".GPC0026 .filter-open-floating .float-sort-box .sort-select .sortBy"
              ).on({
                change: function (e) {
                  if (
                    typeof ePrivacyCookies == "undefined" ||
                    ...
                    $(plp.el.form).submit();
                    $(
                      ".GPC0026 .filter-open-floating .floating-filter"
                    ).removeClass("active");
                    $(".GPC0026 #categoryFilterForm").removeClass("open");
                    $(".GPC0026 .filter-open-box").css("display", "block");
                    $(".GPC0026 .filter-open-floating").removeClass(
      ...
      slider: {
        ...
        init: function () {
          ...
          for (var i = 0; i < _this.el.length; i++) {
            ...
            $(range)
              ...
              .eq(0)
              .attr("aria-label", range.getAttribute("data-min"));
            $(range)
              .find(".ui-slider-handle")
              .eq(1)
              .attr("aria-label", range.getAttribute("data-max"));
        ...
        slide: function (event, ui) {
          ...
          if (
            ...
            ePrivacyCookies.get("LGCOM_IMPROVEMENTS") ||
            COUNTRY_CODE.toLowerCase() == "es" ||
            COUNTRY_CODE.toLowerCase() == "uk" ||
            COUNTRY_CODE.toLowerCase() == "it" ||
            COUNTRY_CODE.toLowerCase() == "fr"
          ) {
            //LGEIS-1124,LGCOMFR-46
            ...
            $wrap
              .find(".ui-slider-handle")
              .eq(0)
              .attr("aria-valuetext", minKey)
              .attr("aria-valuemin", min)
              .attr("aria-valuemax", max)
              .attr("aria-valuenow", minKey);
            $wrap
              .find(".ui-slider-handle")
              .eq(1)
              .attr("aria-valuetext", maxKey)
              .attr("aria-valuemin", min)
              .attr("aria-valuemax", max)
              .attr("aria-valuenow", maxKey);
            // 20200311 END
          ...
          if ($(_this.el).is("div")) {
            for (var i = 0; i < _this.el.length; i++) {
              ...
              valueIndex = {
                ...
                max: valueIndex.max
                  ? valueIndex.max
                  : $(__range).slider("option", "max"),
              };
              $(__this)
                .find(".slider-range")
                .slider("values", [valueIndex.min, valueIndex.max])
                .data("ui-slider")
                ._slide();
            }
          }
        ...
        bake: function (param) {
          ...
          if (_this.cont.join("") == "") {
            ...
            if (
              ...
              //LGEDE-926,LGEIS-1124,LGCOMFR-46
      ...
      init: function () {
        ...
        // LGEAE-386, LGEITF-565 Start
        if ($("[data-del-nofollow-url]").length) {
          var delNofollowUrl = $("#resultProductList").data("delNofollowUrl");
          if (
            delNofollowUrl != "plp-del-nofollow-url" &&
            delNofollowUrl.indexOf(location.pathname) > -1
          ) {
        ...
        // LGEAE-386, LGEITF-565 End

        // 2020.10.06 PJTPLP WhiteSpace
        if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
          filter.labelUseFlag = $("#whiteSpaceFlag").attr(
            "data-label-use-flag"
          );
          filter.useLabelIcon = $("#whiteSpaceFlag").attr(
            "data-use-label-icon"
          );
          ...
          filter.loginUseFlag = $("#reviewDefaultData").attr(
            "data-login-use-flag"
          );
          filter.reviewLoginUrl = $("#reviewDefaultData").attr(
            "data-review-login-url"
          );
        ...
        if (pageHistory.getHistory()) {
          ...
          if (param) {
            ...
            if (page == "viewAll") {
              setTimeout(function () {
                $("#resultProductList .result-info a.view-all").trigger(
                  "click"
                );
              }, 300);
            } else if (parseInt(page) > 1) {
              setTimeout(function () {
                $("#resultAppendTarget .pagination ul li")
                  .eq(parseInt(page) - 1)
                  .find("button")
                  .trigger("click");
              }, 300);
      ...
      getParam: function () {
        ...
        if (
          ...
          if (param.length > 0) {
            ...
            for (var i = 0; i < arr.length; i++) {
              ...
              if (
                typeof $("#categoryFilterForm").find(
                  "[name=" + name + "]"
                )[0] != "undefined"
              ) {
                if (
                  $("#categoryFilterForm").find("[name=" + name + "]")[0]
                    .type == "checkbox" ||
                  !!$("#categoryFilterForm")
                    .find("[name=" + name + "]")
                    .parents(".slider-wrap").length
                ) {
                  hasURLFilter = false;
                  console.log(
                    $("#categoryFilterForm").find("[name=" + name + "]")
                  );
                  if (cnt != 0) {
                    result += "|";
                  }
        ...
        var defaultFilter = $(".GPC0007")
          .find('input[name="defaultFilter"]')
          .val();
        if (
      ...
      setInput: function (param, pageFlag, initMobile) {
        //BTOBGLOBAL-434 : 20210408 modify
        ...
        for (var i = 0; i < paramArray.length; i++) {
          ...
          if (ignore.indexOf(_param[0]) == -1) {
            ...
            if (type == "checkbox") {
              ...
              var val =
                _this.parent().find(".text .name").length > 0
                  ? changeTitleFormat(_this.parent().find(".text .name").text())
                  : changeTitleFormat(_this.parent().find(".text").text());
              var relVal2 =
                _this.parent().find(".text .filter-cnt").length == 0
                  ? _this.parent().find(".text").text()
                  : _this
                      .parent()
                      .find(".text")
                      .text()
                      .replace(
                        _this.parent().find(".text .filter-cnt").text(),
                        ""
                      );
              ...
              if (_this.prop("checked")) {
                ...
                $(
                  ".GPC0007 .apply-filters span[data-filter-value=" +
                    _this.val() +
                    "] a"
                  ...
                  if ($(this).parent().attr("data-filter-value") !== "Y") {
                    $(
                      ".GPC0007 #categoryFilterForm input[value=" +
                        $(this).parent().attr("data-filter-value") +
                        "]"
                    ).click();
                  } else {
                    $(
                      ".GPC0007 #categoryFilterForm input[name=" +
                        $(this).parent().attr("data-filter-name") +
                        "]"
                    ).click();
                  }
                });
                ...
                $(
                  ".GPC0026 .apply-filters span[data-filter-value=" +
                    _this.val() +
                    "] a"
                ).one("click", function (e) {
                  // jslint ignore:line
                  ...
                  if ($(this).parent().attr("data-filter-value") !== "Y") {
                    $(
                      ".GPC0026 #categoryFilterForm input[value=" +
                        $(this).parent().attr("data-filter-value") +
                        "]"
                    ).click();
                  } else {
                    $(
                      ".GPC0026 #categoryFilterForm input[name=" +
                        $(this).parent().attr("data-filter-name") +
                        "]"
                    ).click();
                  }
              ...
              if ($slider.length > 0) {
                ...
                var index = filter.slider.indexOfValue(
                  filter.slider.value[$slider.attr("data-value")],
                  _param[1]
                );
                ...
                if (!val1)
                  val1 = Object.keys(dragbarVal[sliderType][0]).join("");
                if (!val2)
                  val2 = Object.keys(
                    dragbarVal[sliderType][dragbarVal[sliderType].length - 1]
                  ).join("");
                var relVal =
                  _this2.parent().siblings(".title").find("strong").text() +
                  " : " +
                  val1 +
                  "-" +
                  val2;
                if (
                  ...
                  _targetTxt.contents().filter(function () {
                  ...
                  $(
                    ...
                    var options = $(
                      '.GPC0007 .slider-wrap[data-value="' +
                        fv +
                        '"] .slider-range'
                    ).slider("option");
                    $('.GPC0007 .slider-wrap[data-value="' + fv + '"] input')
                      .val("")
                      .removeData("index");
                    $(
                      '.GPC0007 .slider-wrap[data-value="' +
                        fv +
                        '"] .min-box span'
                    ).text(Object.keys(dragbarVal[fv][options.min])[0]);
                    $(
                      '.GPC0007 .slider-wrap[data-value="' +
                        fv +
                        '"] .max-box span'
                    ).text(Object.keys(dragbarVal[fv][options.max])[0]);
                    // 20200325 END
                    $(this).parent().remove();
                  });
                }
                if ($(".GPC0007 .apply-filters span").length != 0) {
                  ...
                  $(".GPC0007 .apply-filters a.link-text").removeClass(
                    "active"
                  );
                }
                ...
                if (
                  ...
                  $(
                    ...
                    var fv = $(this).parent().attr("data-filter-value");
                    var options = $(
                      '.GPC0026 .slider-wrap[data-value="' +
                        fv +
                        '"] .slider-range'
                    ).slider("option");
                    $('.GPC0026 .slider-wrap[data-value="' + fv + '"] input')
                      .val("")
                      .removeData("index");
                    $(
                      '.GPC0026 .slider-wrap[data-value="' +
                ...
                if ($(".GPC0026 .apply-filters span").length != 0) {
                  ...
                  $(".GPC0026 .apply-filters a.link-text").removeClass(
                    "active"
                  );
                }
                // LGEGMC-526 End
      ...
      createProductItem: function (productList, productMessages, data) {
        ...
        if (productList != null && productList.length > 0) {
          ...
          if (emiMsgAreaYn == "Y" || limitSaleAreaYn == "Y") {
            filter.template = filter.template.replace(
              "*isHasTopInfo*",
              "has-topInfo"
            );
          } else {
        ...
        if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
          ...
          if ($(".GPC0026").length > 0 || $(".GPC0007").length > 0) {
        ...
        for (var i = 0; i < productList.length; i++) {
          ...
          if ($(".GPC0134").length > 0 || $(".GPC0142").length > 0) {
            if (i == 0) {
              $("#whiteSpaceData [data-category-id]").attr(
                "data-category-id",
                $("input[name='curationId']").val()
              );
              $("#whiteSpaceData [data-tag-content-area-yn]").attr(
                "data-tag-content-area-yn",
                productList[i].tagContentAreaYn
              );
              $("#whiteSpaceData [data-model-brand-area-yn]").attr(
                "data-model-brand-area-yn",
                productList[i].modelBrandAreaYn
              );
              $("#whiteSpaceData [data-price-area-yn]").attr(
                "data-price-area-yn",
                productList[i].priceAreaYn
              );
              $("#whiteSpaceData [data-promotion-area-yn]").attr(
                "data-promotion-area-yn",
                productList[i].promotionAreaYn
              );
              $("#whiteSpaceData [data-reivew-area-yn]").attr(
                "data-review-area-yn",
                productList[i].reviewAreaYn
              );
              $("#whiteSpaceData [data-energy-fiche-yn]").attr(
                "data-energy-fiche-yn",
                productList[i].energyFicheAreaYn
              );
              $("#whiteSpaceData [data-campaign-area-yn]").attr(
                "data-campaign-area-yn",
                productList[i].campaignAreaYn
              );
              $("#whiteSpaceData [data-btn-area-yn]").attr(
                "data-btn-area-yn",
                productList[i].btnAreaYn
              );
              $("#whiteSpaceData [data-emi-msg-area-yn]").attr(
                "data-emi-msg-area-yn",
                productList[i].emiMsgAreaYn
              );
              $("#whiteSpaceData [data-emi-use-flag]").attr(
                "data-emi-use-flag",
                productList[i].emiUseFlag
              );
              $("#whiteSpaceData [data-spec-msg-flag-area-yn]").attr(
                "data-spec-msg-flag-area-yn",
                productList[i].specMsgFlagAreaYn
              );
              $("#resultAppendTarget").attr(
                "categoryId",
                $("input[name='curationId']").val()
              );
            }
          ...
          template = template
            ...
            .replace(/\*productTag3\*/g, p.productTag3) /* PJTQUICKWIN */
            .replace(
              /\*productTag1UserType\*/g,
              p.productTag1UserType
            ) /* LGEDE-354 */
            .replace(
              /\*productTag2UserType\*/g,
              p.productTag2UserType
            ) /* LGEDE-354 */
            .replace(
              /\*productTag3UserType\*/g,
              p.productTag3UserType
            ) /* PJTQUICKWIN */
            ...
            .replace(
              /\*promotionText\*/g,
              p.promotionText ? p.promotionText : ""
            )
            /* LGEPL-80 */
            .replace(
              /\*promotionLinkUrl\*/g,
              p.promotionLinkUrl ? p.promotionLinkUrl : ""
            )
            ...
            .replace(
              /\*rDiscountedPrice\*/g,
              p.rDiscountedPrice
                ? changeFormatPrice(p.rDiscountedPrice)
                : "null"
            )
            ...
            .replace(
              /\*icoFav\*/g,
              p.myWishCnt == "Y" ? "ico-fav on" : "ico-fav"
            )
            ...
            .replace(
              /\*emiPopUrl\*/g,
              p.obsEmiMsgFlag == "Y" && p.emiPopUrl != null ? p.emiPopUrl : ""
            )
            .replace(
              /\*emiMsg\*/g,
              p.obsEmiMsgFlag == "Y" && p.emiMsg != null && p.emiMsg != ""
                ? p.emiMsg
                : ""
            )
            //PJTOBSB2E-3 End
            .replace(
              /\*calculatorSalesCode\*/g,
              p.salesModelCode != null &&
                p.salesModelCode != "" &&
                p.salesSuffixCode != null &&
                p.salesSuffixCode != ""
                ? p.salesModelCode + "." + p.salesSuffixCode
                : p.salesModelCode
            ) // LGEGMC-2434, LGEGMC-2589
            //LGEGMC-1406
            .replace(
              /\*specMsg\*/g,
              p.specMsgFlag == "Y" ? "<p>" + specMsg + "</p>" : ""
            )
            //PJTOBSB2E-3 End
            .replace(/\*msrp\*/g, nvl(p.msrp, "0"))
            .replace(
              /\*membershipPrice\*/g,
              changeFormatFullPrice(p.rMembershipPrice, p.rMembershipPriceCent)
            ) /* PJTMEMBERSHIP-4 */
            .replace(
              /\*membershipDisplayFlag\*/g,
              p.membershipDisplayFlag
            ) /* PJTMEMBERSHIP-4 */
            .replace(
              /\*cheaperPrice\*/g,
              changeFormatFullPrice(p.cheaperPrice, p.cheaperPriceCent)
            ) /* LGEGMC-1973 */
            .replace(
              /\*cheaperPriceFlag\*/g,
              p.cheaperPriceFlag
            ...
            .replace(
              /\*afterPayInstallMent\*/g,
              p.obsEmiMsgFlag == "Y" &&
                p.emiMsg != null &&
                p.emiMsg != "" &&
                p.afterPay <= 3000 &&
                p.afterPay > 0
                ? 'afterpay-installment" href="#modal-afterpay'
                : '" style="display:none;'
            ) /* LGEAU-378 add */
            .replace(
              /\*wtbClass\*/g,
            ...
            .replace(
              /\*isHasMember\*/g,
              hasMemberFlag == "Y" ? "has-member" : ""
            ) /* PJTMEMBERSHIP-11 */
            .replace(
              /\*zipPayInstallMent\*/g,
              p.obsEmiMsgFlag == "Y" && p.afterPay > 0 && p.afterPay <= 10000
                ? 'afterpay-installment" href="#modal-afterpay'
                : '" style="display:none;'
            ) /* LGEGMC-3167 add, OBSLGEAU-749 (5000 -> 10000) */
            .replace(
              /\*orZipPay\*/g,
              p.obsEmiMsgFlag == "Y" && p.afterPay > 0 && p.afterPay <= 3000
                ? "or "
                : ""
            ) /* LGEGMC-3167 add */;
          ...
          if (
            p.obsMembershipLinkUrl != "" &&
            p.obsMembershipLinkUrl != null &&
          ...
          template = template.replace(
            /\*obsMemberShipLinkStart\*/g,
            obsMemberShipLinkStartHtml
          );
          template = template.replace(
            /\*obsMemberShipLinkEnd\*/g,
          ...
          var productFicheDownload = $(
            'input[name="productFicheDownload"]'
          ).val();
          ...
          if (
            ...
            var productFicheDownload = $(
              'input[name="productNewFicheDownload"]'
            ).val();
          } else if (docTypeCodeFlag == "O") {
            var productFicheDownload = $(
              'input[name="productOldFicheDownload"]'
            ).val();
          }
          //LGEGMC-5315
          ...
          if (p.washTowerFlag == "Y") {
            if (
              p.productFicheFileName != "" &&
              p.productFicheOriginalName != "" &&
              p.productFicheFileName != null &&
              p.productFicheOriginalName != null &&
              p.productFicheDocId != null &&
              p.productFicheDocId != "" &&
              p.secondProductFicheFileName != "" &&
              p.secondProductFicheOriginalName != "" &&
              p.secondProductFicheFileName != null &&
              p.secondProductFicheOriginalName != null &&
              p.secondProductFicheDocId != null &&
              p.secondProductFicheDocId != ""
            ) {
              productFichehtml +=
              ...
              productFichehtml +=
                ...
                pdfDownloadFile +
                "(" +
                p.productFicheproductLeve1Code +
                ")" +
                "'>" +
                p.productFicheproductLeve1Code +
                "</a>";
              productFichehtml +=
                ...
                pdfDownloadFile +
                "(" +
                p.secondProductFicheproductLeve1Code +
                ")" +
                "'>" +
                p.secondProductFicheproductLeve1Code +
                "</a>" +
          ...
          if ($("html").attr("data-countrycode") == "uk") {
            if (
              ...
              p.energyLabel != "N" &&
              p.energyLabel != null &&
              p.energyLabelDocId != null &&
              p.energyLabelDocId != "" &&
              p.energyLabelFileName != null &&
              p.energyLabelFileName != "" &&
              p.energyLabelOriginalName != null &&
              p.energyLabelOriginalName != "" &&
              p.energyLabelImageAddr != null &&
              p.energyLabelImageAddr != "" &&
              p.energyLabelName != null &&
              p.energyLabelName != "" &&
              p.fEnergyLabelFileName != null &&
              p.fEnergyLabelDocId != null &&
              p.fEnergyLabelDocId != "" &&
              p.fEnergyLabelFileName != "" &&
              p.fEnergyLabelOriginalName != null &&
              p.fEnergyLabelOriginalName != ""
            ) {
              var energyLabelhtml =
              ...
              if (
                ...
                energyLabelImagehtml +=
                  ...
                  pdfDownloadFile +
                  "(" +
                  p.fenergyLabelproductLeve1Code +
                  ")" +
                  "'>" +
              ...
              var energyLabelhtml =
                ...
                "' data-original='" +
                p.energyLabelOriginalName +
                "' class='link-text' data-category='' title='" +
                pdfDownloadFile +
                "(" +
                p.energyLabelproductLeve1Code +
                ")" +
          ...
          if (p.washTowerFlag == "Y") {
            if ($("html").attr("data-countrycode") == "uk") {
              if (
                p.secondEnergyLabel != "" &&
                p.secondEnergyLabel != "N" &&
                p.secondEnergyLabel != null &&
                p.secondEnergyLabelDocId != null &&
                p.secondEnergyLabelDocId != "" &&
                p.secondEnergyLabelFileName != null &&
                p.secondEnergyLabelFileName != "" &&
                p.secondEnergyLabelOriginalName != null &&
                p.secondEnergyLabelOriginalName != "" &&
                p.secondEnergyLabelImageAddr != null &&
                p.secondEnergyLabelImageAddr != "" &&
                p.secondEnergyLabelName != null &&
                p.secondEnergyLabelName != "" &&
                p.secondFEnergyLabelFileName != null &&
                p.secondFEnergyLabelDocId != null &&
                p.secondFEnergyLabelDocId != "" &&
                p.secondFEnergyLabelFileName != "" &&
                p.secondFEnergyLabelOriginalName != null &&
                p.secondFEnergyLabelOriginalName != ""
              ) {
                var secondEnergyLabelhtml =
                  ...
                  p.secondEnergyLabelName +
                  "'></span></a>";
                var secondEnergyLabelImagehtml =
                  "<div class='tooltip-link'><div class='tolltip-inner'>";
                secondEnergyLabelImagehtml +=
                  "<a href='#' class='link-text link-text-uk' adobe-click='pdp-file-down-click' data-doc='" +
                  p.secondFEnergyLabelDocId +
                  "' data-file='" +
                  p.secondFEnergyLabelFileName +
                  ...
                  pdfDownloadFile +
                  "(" +
                  p.secondFEnergyLabelproductLeve1Code +
                  ")" +
                  "'>" +
                  $("#pdfDownloadFileUk").val() +
                  "</a>";
                secondEnergyLabelImagehtml +=
                  ...
                  pdfDownloadFile +
                  "(" +
                  p.secondEnergyLabelproductLeve1Code +
                  ")" +
                  "'>" +
                  $("#pdfDownloadFileEu").val() +
                  "</a></div></div></div>";
          ...
          if (pdrCompareUseFlag == "N") {
            $template.find("a.js-compare").css("display", "none");
          }
          ...
          /* LGCOMPA-44 start, LGCOMITF-39 data undefined check */
          if (
            SIGN_IN_STATUS == "N" &&
          ...
          for (var i1 = 0; i1 < $keyBlocks.length; i1++) {
            ...
            if (!val || val == null || val == "N") {
              if ($currentKeyBlock.is(".btn")) {
                ...
                if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
                  if (
                    (p.obsLimitSale == "Y" && p.limitSaleUseFlag == "Y") ||
                    p.obsPreOrderFlag == "Y" ||
                    p.obsPreOrderRSAFlag == "Y"
                  ) {
                  ...
                  if (
                    p.obsLimitSale != "Y" &&
                    (p.productTag1ColorFlag == "Y" ||
                      p.productTag2ColorFlag == "Y")
                  ) {
                    switch ($template.find(".tag-content p").length) {
                      case 1:
                        if (
                          ...
                          $($template.find(".tag-content p")[0]).addClass(
                            "tag-gold"
                          );
                        }
                        if (
                      ...
                      case 2:
                        ...
                        if (
                          ...
                          $($template.find(".tag-content p")[1]).addClass(
                            "tag-gold"
                          );
                        }
                        break;
                  ...
                  if (
                    $template.find(".model-buy").length > 0 &&
                    p.obsLimitSale == "Y" &&
                    p.limitSaleUseFlag == "Y"
                  ) {
                ...
                if (
                  (key == "productTag1" ||
                    key == "productTag2" ||
                    key == "productTag3") &&
                  ($(".GPC0007").length > 0 || $(".GPC0026").length > 0)
                ) {
          ...
          if ($(".GPC0026").length > 0) {
            ...
            var campaignLinkBtn = $campaignArea
              .find('template[data-campaignbtn="LINK"]')
              .clone()
              .html();

            var $ratingArea = $template.find(".has-review-pop");
            var reviewBtnBV = $ratingArea
              .find('template[data-review-btn-type="BV"]')
              .clone()
              .html();
            var reviewBtnLGCOM = $ratingArea
              .find('template[data-review-btn-type="LGCOM"]')
              .clone()
              .html();
            var reviewBtnSP = $ratingArea
              .find('template[data-review-btn-type="SP"]')
              .clone()
              .html();
            var reviewBtnCENEO = $ratingArea
              .find('template[data-review-btn-type="CENEO"]')
              .clone()
              .html();
            var reviewBtnTxt = $ratingArea.attr("data-review-btn-text");
            var chkWriteReview = $("#reviewDefaultData").attr("data-check");
            var reviewLoginUrl = $("#reviewDefaultData").attr(
              "data-review-login-url"
            );
            ...
            if (p.tooltipBodyFontOpt == "I")
              tmpTooltipBodyFontOpt = " text-italic";
            if (p.tooltipBodyFontOpt == "B")
              tmpTooltipBodyFontOpt = " text-bold";

            if (p.campaignUseFlag == "Y") {
              tmpText =
                "<a href='" +
                p.linkUrl +
                "' role='button' class='btn-campaign' target='" +
                tmpUrlTarget +
                "'>";
              ...
              tmpText = tmpText + "</a>";
              campaignLinkBtn = campaignLinkBtn.replace(
                /\*alinktext\*/g,
                tmpText
              );
            ...
            if (p.reviewBtnUseFlag == "Y" && p.reviewUseFlag == "Y") {
              if (reviewType == "BV" || reviewType == "BV2") {
                tmpBtnText =
                  "<a href='#' data-toggle='modal' role='button' data-review-event-click='BV' class='btn-popup-review' data-review-model-id='" +
                  p.modelId +
                  "' data-adobe-tracking-wish='Y' data-page-event='promotion_plp_review_move'>" +
                  reviewBtnTxt +
                  "</a>";
                reviewBtnBV = reviewBtnBV.replace(
                  /\*reviewBtnText\*/g,
                  tmpBtnText
                );
                ...
                //PJTGADL-2
                tmpBtnText =
                  "<a href='#' data-toggle='modal' role='button' data-review-event-click='LGCOM' class='btn-popup-review' data-review-model-id='" +
                  p.modelId +
                  "' data-bu= '" +
                  p.buName1 +
                  ...
                  "' data-model-id= '" +
                  p.modelId +
                  "' data-model-overallscore= '" +
                  p.reviewRatingStar2 +
                  "' data-model-reviewCnt= '" +
                  p.reviewRating +
                  "' data-review-login-url= '" +
                  reviewLoginUrl +
                  "' data-review-participantCount= '" +
                  p.reviewRating +
                  "' data-review-starRatingPercent='" +
                  p.reviewRatingPercent +
                  "' data-review-starRatingValue='" +
                  p.reviewRatingStar2 +
                  "' data-check='" +
                  chkWriteReview +
                  "' data-adobe-tracking-wish='Y' data-page-event='promotion_plp_review_move'>" +
                  reviewBtnTxt +
                  "</a>";
                reviewBtnLGCOM = reviewBtnLGCOM.replace(
                  /\*reviewBtnText\*/g,
                  tmpBtnText
                );
                ...
                tmpBtnText =
                  "<a href='" +
                  p.modelUrlPath +
                  "'#pdp_review' data-review-event-click='SP' data-review-model-id='" +
                  p.modelId +
                  "' class='btn-popup-review' data-adobe-tracking-wish='Y' data-page-event='promotion_plp_review_move'>" +
                  reviewBtnTxt +
                  "</a>";
                reviewBtnSP = reviewBtnSP.replace(
                  /\*reviewBtnText\*/g,
                  tmpBtnText
                );
                ...
                tmpBtnText =
                  "<a href='" +
                  p.modelUrlPath +
                  "'#pdp_review' data-review-event-click='CENEO' data-review-model-id='" +
                  p.modelId +
                  "' class='btn-popup-review' data-adobe-tracking-wish='Y' data-page-event='promotion_plp_review_move'>" +
                  reviewBtnTxt +
                  "</a>";
                reviewBtnCENEO = reviewBtnCENEO.replace(
                  /\*reviewBtnText\*/g,
                  tmpBtnText
                );
          ...
          if ($priceArea.length != 0) {
            ...
            if (p.modelStatusCode == "DISCONTINUED") {
              ...
              var price = changeFormatFullPrice(
                p.rPromoPrice,
                p.rPromoPriceCent
              );
              ...
              if (p.bizType == "B2B" && p.obsLoginFlag != "Y") {
                // PJTB2BOBS-1
            ...
            if (p.vipPriceFlag == "Y") {
              ...
              var legal =
                p.discountMsg == null
                  ? ""
                  : p.discountMsg.replace(/&lt;/gi, "<").replace(/&gt;/gi, ">"); // LGEIS-229 change how discounts are shown
          ...
          if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
            if (labelUseFlag == "Y" && useLabelIcon == "Y") {
              ...
              var labelAward = $labelList
                .find('template[data-label-type="AWARD"]')
                .clone()
                .html();
              var labelFeature = $labelList
                .find('template[data-label-type="FEATURE"]')
                .clone()
                .html();
              var labelDelivery = $labelList
                .find('template[data-label-type="DELIVERY"]')
                .clone()
                .html();
              var labelWarranty = $labelList
                .find('template[data-label-type="WARRANTY"]')
                .clone()
                .html();
              var labelBullet = $labelList
                .find('template[data-label-type="BULLET"]')
                .clone()
                .html();
              //LGEDE-220
              if ($(".GPC0007").length > 0) {
                var labelShippingFlag = $labelList
                  .find('template[data-label-type="SHIPPINGFLAG"]')
                  .clone()
                  .html();
                var labelDeliveryFlag = $labelList
                  .find('template[data-label-type="DELIVERYFLAG"]')
                  .clone()
                  .html();
                var labelInstallationFlag = $labelList
                  .find('template[data-label-type="INSTALLATIONFLAG"]')
                  .clone()
                  .html();
                var keyfeatureFlag = $labelList
                  .find('template[data-label-type="KEYFEATUREFLAG"]')
                  .clone()
                  .html(); //LGEIN-399 : 20210607 add
              }
              if (productType != "card") {
                // card type에서는 label 사용하지 않음
                ...
                for (var cnt = 0; cnt < labelIconMap.length; cnt++) {
                  ...
                  /* 2020.10.12 PJTPLP 수정 */
                  if (
                    labelIconMap[cnt].shortDescType == "AWARD" ||
                    labelIconMap[cnt].shortDescType == "FEATURE"
                  ) {
                    if (
                      labelIconMap[cnt].cssFontBold == "Y" &&
                      labelIconMap[cnt].cssFontItalic == "Y"
                    ) {
                      tmpText =
                    ...
                    if (
                      labelIconMap[cnt].linkUrl != null &&
                      labelIconMap[cnt].linkUrl != ""
                    ) {
                      tmpText =
                        tmpText +
                        "<a href='" +
                    ...
                    if (
                      labelIconMap[cnt].imagePathAddr != "" &&
                      labelIconMap[cnt].imagePathAddr != null
                    ) {
                      tmpText =
                        tmpText +
                        "<img src='" +
                        labelIconMap[cnt].imagePathAddr +
                        "' alt='" +
                        labelIconMap[cnt].altText +
                        "' aria-hidden='true'>" +
                        "<p>" +
                        labelIconMap[cnt].shortDesc +
                        "</p>";
                      if (
                        labelIconMap[cnt].linkUrl != null &&
                        labelIconMap[cnt].linkUrl != ""
                      ...
                      tmpText = tmpText + "</li>";
                      tmpLabelAward = tmpLabelAward.replace(
                        /\*shortDesc\*/g,
                        tmpText
                      );
                      ...
                      tmpText =
                        tmpText + "<p>" + labelIconMap[cnt].shortDesc + "</p>";
                      if (
                        labelIconMap[cnt].linkUrl != null &&
                        labelIconMap[cnt].linkUrl != ""
                      ) {
                      ...
                      tmpText = tmpText + "</li>";
                      tmpLabelFeature = tmpLabelFeature.replace(
                        /\*shortDesc\*/g,
                        tmpText
                      );
                    ...
                    tmpLabelDelivery = tmpLabelDelivery.replace(
                      /\*shortDesc\*/g,
                      labelIconMap[cnt].shortDesc
                    );
                    $labelList.append(tmpLabelDelivery);
                  } else if (labelIconMap[cnt].shortDescType == "WARRANTY") {
                    tmpLabelWarranty = tmpLabelWarranty.replace(
                      /\*shortDesc\*/g,
                      labelIconMap[cnt].shortDesc
                    );
                    ...
                    if ($(".GPC0007").length > 0) {
                      tmpLabelShippingFlag = tmpLabelShippingFlag.replace(
                        /\*shortDesc\*/g,
                        labelIconMap[cnt].shortDesc
                      );
                    ...
                    labelIconMap[cnt].shortDescType == "DELIVERYFLAG"
                  ) {
                    if ($(".GPC0007").length > 0) {
                      tmpLabelDeliveryFlag = tmpLabelDeliveryFlag.replace(
                        /\*shortDesc\*/g,
                        labelIconMap[cnt].shortDesc
                      );
                    ...
                    labelIconMap[cnt].shortDescType == "INSTALLATIONFLAG"
                  ) {
                    if ($(".GPC0007").length > 0) {
                      tmpLabelInstallationFlag =
                        tmpLabelInstallationFlag.replace(
                          /\*shortDesc\*/g,
                          labelIconMap[cnt].shortDesc
                        );
                  ...
                  //LGEDE-220
                    ...
                    tmpLabelBullet = tmpLabelBullet.replace(
                      /\*shortDesc\*/g,
                      labelIconMap[cnt].shortDesc
                    );
                    ...
                    labelIconMap[cnt].shortDescType == "KEYFEATUREFLAG"
                  ) {
                    if ($(".GPC0007").length > 0) {
                      if (
                        labelIconMap[cnt].cssFontBold == "Y" &&
                      ...
                      tmpLabelKeyfeatureFlag = tmpLabelKeyfeatureFlag
                        .replace(/\*shortDesc\*/g, labelIconMap[cnt].shortDesc)
                        .replace(/\*cssFont\*/g, tmpText);
              ...
              if (productType == "card") {
                // card type
                if (!!p.signatureFlag && p.signatureFlag == "Y") {
                  ynBrandTag = "Y";
                } else {
                  $template.find(".model-brand-area .brand-signature").remove();
                }
                if (!!p.thinqFlag && p.thinqFlag == "Y") {
                  ynBrandTag = "Y";
                } else {
                  $template.find(".model-brand-area .brand-thinq").remove();
                }
              } else {
                // default
                var $tempThinqSignatureFlag = $template.find(".model-brand");
                var thinqFlag = $tempThinqSignatureFlag
                  .find("template[data-thinqFlag]")
                  .clone()
                  .html();
                var signatureFlag = $tempThinqSignatureFlag
                  .find("template[data-signatureFlag]")
                  .clone()
                  .html();
          ...
          if (p.siblingType && p.siblingType != null) {
            siblingTypeclass =
              p.siblingType.toLowerCase() == "color" ? "color" : "size";
          ...
          if (havSiblingModels && siblingTypeclass != null && $sibling.get(0)) {
            var siblingItem = $template
                .find(".model-group ." + siblingTypeclass)
                .clone()
                .html(),
              siblingAriaTxt = $template
                .find(".model-group ." + siblingTypeclass)
                .clone()
                .attr("aria-label"),
              siblingMarkup = [];
            ...
            for (var _j = 0; _j < p.siblingModels.length; _j++) {
              ...
              var item = siblingItem
                .replace(/\*siblingCode\*/g, sbModel.siblingCode)
            ...
            $template
              .find(".model-group .inner")
              .attr("aria-label", siblingAriaTxt);

            // PJTQUICKWIN 20220225 START - add
            if (productType == "card") {
              if (siblingTypeclass == "color") {
                $template.find(".model-group").addClass("type-color");
              } else {
                $template.find(".model-group").addClass("type-size");
              }
            }
          ...
          // rolling image
          if (productType == "card") p.modelRollingImgList = null; // card type 에서는 이미지 rolling 기능 사용하지 않음
          if (p.modelRollingImgList && p.modelRollingImgList != null) {
            $template
              .find(".visual img.pc")
              .addClass("js-thumbnail-loop")
              .attr("data-img-list", p.modelRollingImgList);
          }
          ...
          //PJTLIMITQTY-1 START
          if (
            (!p.reStockAlertFlag || p.reStockAlertFlag != "Y") &&
            $stockArea.length > 0
          ) {
            if (
              ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) &&
              ...
              if (p.obsInventoryFlag == "Y") {
                ...
                $stockArea
                  .find(".text")
                  .text(productMessages.limitSaleSoldOutText);
              }
            } else {
              $stockArea.removeClass("out-of-stock").empty();
            }
          ...
          if (productType == "card") {
            ...
            if ($template.find("a.re-stock-alert").length > 0) {
              if (!p.reStockAlertFlag || p.reStockAlertFlag != "Y") {
                $template.find("a.re-stock-alert").addClass("hidden");
              } else {
                $template.find("a.re-stock-alert").removeClass("hidden");
              }
            }
            ...
            if (p.addToCartFlag == "Y") {
              ...
              if (p.obsBuynowFlag == "Y") {
                ...
                // $template.find('a.add-to-cart').removeClass('hidden').data('model-id', p.modelId).attr('href', p.modelUrlPath).text(buynow).removeAttr('target, title');
                $template.find("a.add-to-cart").addClass("hidden");
              } else {
                // 통합 OBS
                $template
                  .find("a.add-to-cart")
            ...
            if (
              p.installationFlag == "Y" &&
              p.installationFlag != null &&
              p.installationFlag != ""
            ) {
              $template.find("js-installration").removeClass("hidden");
            } else {
              $template.find("js-installration").addClass("hidden");
            }
            ...
            if (
              p.inquiryToBuyFlag == "Y" &&
              p.inquiryToBuyUrl != null &&
              p.inquiryToBuyUrl != ""
            ) {
              $template
                .find("a.inquiry-to-buy")
                .removeClass("hidden")
                .attr("href", p.inquiryToBuyUrl)
                .text(productMessages.inquiryToBuyBtnNm);
            } else {
              $template.find("a.inquiry-to-buy").addClass("hidden");
            }
            ...
            if (
              ...
              $template
                ...
                .text(productMessages.whereToBuyBtnNm)
                .parent()
                .removeClass("hidden");
              $template
                .find("a.where-to-buy")
              ...
              $template.find("a.where-to-buy").parent().addClass("hidden");
            }

            // Find a dealer  (top area - 미출력시 상위 li를 지워야 함)
            if (
              p.findTheDealerFlag == "Y" &&
              p.findTheDealerUrl != null &&
              p.findTheDealerUrl != ""
            ) {
              $template
                .find("a.find-a-dealer")
                .attr("href", p.findTheDealerUrl)
                .text(productMessages.findTheDealerBtnNm)
                .parent()
                .removeClass("hidden");
            } else {
              $template.find("a.find-a-dealer").parent().addClass("hidden");
            }
          } else {
            // default type
            ...
            if (p.obsPreOrderFlag == "Y") {
              ...
              if (p.obsBuynowFlag == "Y") {
                $template
                  ...
                  .text(productMessages.preOrderBtnNm)
                  .removeAttr("target, title")
                  .addClass("pre-order")
                  .attr("data-obs-pre-order-start-date", p.obsPreOrderStartDate)
              ...
              if (p.addToCartFlag == "Y") {
                ...
                if (p.obsBuynowFlag == "Y") {
                  ...
                  $template
                    ...
                    .data("model-id", p.modelId)
                    .attr("href", "#")
                    .text(productMessages.addToCartBtnNm)
                    .attr("role", "button")
                    .removeAttr("target, title");
                }
              } else if (p.addToCartFlag == "S") {
              ...
              if (p.ecommerceTarget == "_blank") {
                $template
                  ...
                  .text(productMessages.buyNowBtnNm)
                  .removeAttr("role")
                  .attr("target", "_blank")
                  .attr("title", productMessages.btnNewLinkTitle); //LGEGMC-1567
                /*LGEGMC-1841 s*/
                ...
                $template
                  ...
                  .text(productMessages.buyNowBtnNm)
                  .removeAttr("role")
                  .removeAttr("target, title"); //LGEGMC-1567
              }
              ...
              $template
                ...
                .text(productMessages.resellerBtnNm)
                .removeAttr("role")
                .attr("target", "_blank")
                .attr("title", productMessages.btnNewLinkTitle);
              ...
              $template
                ...
                .text(productMessages.productSupportBtnNm)
                .removeAttr("role")
                .removeAttr("target, title");
            } else {
            ...
            // LGEUK-896 Start
            if ($template.find(".button a.recommended-products").length > 0) {
              if (p.obsSellFlag != "Y") {
                $template
            ...
            if (
              ...
              $template
                .find(".button a.in-buynow:not(.add-to-cart)")
                .addClass("active")
                .attr("href", p.wtbExternalLinkUrl)
                .text(p.wtbExternalLinkName)
                .attr("data-link-name", "buy_now")
          ...
          if (i == 0) {
            // accessiblity focus item
            $template.addClass("first-item");
          }
          /* 2020.10.12 PJTPLP 수정 */
          if (
            (p.rPrice != 0 && p.rPrice != null) ||
            (p.rPromoPrice != 0 && p.rPromoPrice != null) ||
            p.retailerPricingFlag == "Y" ||
            p.reStockAlertFlag == "Y"
          ) {
            ynPrice = "Y";
          }
          if (
            (p.reviewRating != "0" && p.reviewRating != null) ||
            (p.reviewRatingStar != "0" && p.reviewRatingStar != null) ||
            (p.reviewRatingStar2 != "0.0" && p.reviewRatingStar2 != null) ||
          ...
          if (!!data) {
            if (
              data.recommendedRetailRriceInfo == "Y" &&
              (!!p.rPromoPrice || !!p.rPrice)
            ) {
        ...
        if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
          var template = html;
          var $template = $(template);
          var $modelButtonEl = $template.find(
            ".products-info .model-button .button"
          );
          $modelButtonEl.removeClass("only-button");
          $modelButtonEl.addClass("only-button");
      ...
      loadPLP: function (e, pageFlag, initMobile) {
        // BTOBGLOBAL-434 : 20210408 modify
        var form = filter.el;
        ...
        ajax.call(
          ...
          function (d) {
            ...
            if (d.status == "success" && data) {
              ...
              if (
                $(filter.result.querySelector(".no-content")).is(":visible")
              ) {
                $(filter.result).children().removeAttr("style");
              }

              data.productList =
                data.productList && data.productList != null
                  ? data.productList
                  : [];
              data.multiCategorys =
                data.multiCategorys && data.multiCategorys != null
                  ? data.multiCategorys
                  : [];
              /* LGEUS-11775 : 20190807 add, LGEUS-11780 20190812 add */
              if (
                data.promotionTotalCount != null &&
                filter.el.querySelector(
                  "input[type=checkbox][name=promotionsOnly]"
                ) != null &&
                data.promotionTotalCount == 0
              ) {
                var promotionsOnlySelector1 = filter.el.querySelector(
                  "input[type=checkbox][name=promotionsOnly]"
                );
                promotionsOnlySelector1.setAttribute("disabled", "disabled");
                promotionsOnlySelector1.parentNode.classList.add("disabled");
                $(promotionsOnlySelector1)
                  .siblings(".filter-cnt")
                  .text("(" + data.promotionTotalCount + ")");
              } else if (
                data.promotionTotalCount != null &&
                filter.el.querySelector(
                  "input[type=checkbox][name=promotionsOnly]"
                ...
                var promotionsOnlySelector2 = filter.el.querySelector(
                  "input[type=checkbox][name=promotionsOnly]"
                );
                promotionsOnlySelector2.removeAttribute("disabled");
                promotionsOnlySelector2.parentNode.classList.remove("disabled");
                $(promotionsOnlySelector2)
                  .siblings(".filter-cnt")
                  .text("(" + data.promotionTotalCount + ")");
              }

              if (
                data.obsTotalCount != null &&
                filter.el.querySelector("input[type=checkbox][name=obsOnly]") !=
                  null &&
                data.obsTotalCount == 0
              ) {
                var obsOnlySelector1 = filter.el.querySelector(
                  "input[type=checkbox][name=obsOnly]"
                );
                obsOnlySelector1.setAttribute("disabled", "disabled");
                obsOnlySelector1.parentNode.classList.add("disabled");
                $(obsOnlySelector1)
                  .siblings(".filter-cnt")
                  .text("(" + data.obsTotalCount + ")");
              } else if (
                data.obsTotalCount != null &&
                filter.el.querySelector("input[type=checkbox][name=obsOnly]") !=
                  null &&
                ...
                var obsOnlySelector2 = filter.el.querySelector(
                  "input[type=checkbox][name=obsOnly]"
                );
              ...
              if (
                data.bundlesTotalCount != null &&
                filter.el.querySelector(
                  "input[type=checkbox][name=bundlesOnly]"
                ) != null &&
                data.bundlesTotalCount == 0
              ) {
                var bundlesOnlySelector1 = filter.el.querySelector(
                  "input[type=checkbox][name=bundlesOnly]"
                );
                bundlesOnlySelector1.setAttribute("disabled", "disabled");
                bundlesOnlySelector1.parentNode.classList.add("disabled");
                $(bundlesOnlySelector1)
                  .siblings(".filter-cnt")
                  .text("(" + data.bundlesTotalCount + ")");
              } else if (
                data.bundlesTotalCount != null &&
                filter.el.querySelector(
                  "input[type=checkbox][name=bundlesOnly]"
              ...
              if (
                $(filter.result.querySelector(".promotion-wrapper")).length ==
                  0 &&
                (data.productList.length > 0 || data.multiCategorys.length == 1)
              ) {
                ...
                if (data.multiCategorys && data.multiCategorys.length == 1) {
                  ...
                  data =
                    singleCate instanceof Array ? singleCate[0] : singleCate;
                  data.filterEnableList = filterEnableList;
                }

                // make markup
                //console.log(data);
                html = filter.createProductItem(
                  data.productList,
                  d.data[0].productMessages,
                  data
                );
                ...
                if (isExpander) {
                  ...
                  if (pageVisible && pageInfo.page < pageInfo.pageCount) {
                    filter.result.querySelector(".expander button").value =
                      pageInfo.page + 1;
                  ...
                  if (typeof renderListingInlineRatingsRU != "undefined")
                    renderListingInlineRatingsRU(getProductsNameRU());
                  if (data.totalCount) {
                    $("#resultProductList .result-number").text(
                      data.totalCount
                    );
                  ...
                  if (
                    $("#categoryFilterForm input[type=checkbox]:checked")
                      .length == 0 &&
                    $('#categoryFilterForm .slider-wrap input[value!=""]')
                      .length == 0
                  ) {
                  ...
                  if (
                    Number(
                      $(
                        "#resultProductList .total2 .result-number-total"
                      ).text()
                    ) <
                    Number(
                      $("#resultProductList .total2 .result-number").text()
                    )
                  ) {
                    $("#resultProductList .total2 .result-number-total").text(
                      Number(
                        $("#resultProductList .total2 .result-number").text()
                      )
                    );
                  ...
                  if (pageInfo && pageInfo.view == "Y") {
                    if (filter.result.querySelector(".pagination")) {
                      var pageMarkup = [];
                      for (
                        var pageIdx = pageInfo.loopStart;
                        pageIdx <= pageInfo.loopEnd;
                        pageIdx++
                      ) {
                        ...
                        if (
                          filter.result
                            ...
                            .getAttribute("data-aria-pattern")
                        ) {
                          var _ariaText = filter.result
                            .querySelector(".pagination")
                            .getAttribute("data-aria-pattern")
                            .replace(/\#/g, pageIdx);
                      ...
                      $(filter.result.querySelector(".pagination ul")).html(
                        pageMarkup
                      );

                      filter.result.querySelector(
                        ".pagination .prev"
                      ).disabled = !pageInfo.leftPage;
                      filter.result.querySelector(
                        ".pagination .next"
                      ).disabled = !pageInfo.rightPage;
                      filter.result.querySelector(".pagination .prev").value =
                        pageInfo.page - 1;
                      filter.result.querySelector(".pagination .next").value =
                        pageInfo.page + 1;
                      $(filter.result.querySelector(".pagination")).show();
                    } else if (
                      filter.result.querySelectorAll(".btn-area").length > 0
                    ) {
                      filter.result.querySelector(".btn-area .prev").disabled =
                        !pageInfo.leftPage;
                      filter.result.querySelector(".btn-area .next").disabled =
                        !pageInfo.rightPage;
                    ...
                    if (!!!filter.result.querySelector(".pagination")) {
                      ...
                      var pageMarkup = [];
                      pageTemplate +=
                        '<div class="pagination" role="navigation" aria-label="' +
                        $("input[name=component-Pagination]").val() +
                        '" data-aria-pattern="' +
                        $("input[name=component-page]").val() +
                        ' #">';
                      pageTemplate +=
                        '<button type="button" value="1" class="prev" aria-label="' +
                        $("input[name=component-previousPage]").val() +
                        '"' +
                        prevFlag +
                        ">" +
                        $("input[name=component-prev]").val() +
                        "</button>";
                      ...
                      pageTemplate += "</ul>";
                      pageTemplate +=
                        '<button type="button" value="2" class="next" aria-label="' +
                        $("input[name=component-nextPage]").val() +
                        '"' +
                        nextFlag +
                        ">" +
                        $("input[name=component-next]").val() +
                        "</button>";
                      pageTemplate += "</div>";
                    ...
                    if (
                      pageInfo.loopStart != pageInfo.loopEnd &&
                      !!!filter.result.querySelector(".expander")
                    ) {
                      var pageTemplate = "";
                      pageTemplate += '<div class="expander">';
                      pageTemplate +=
                        '<button type="button" class="btn btn-outline-secondary btn-sm" value="2">' +
                        $("input[name=component-loadMore]").val() +
                        "</button>";
                      pageTemplate += "</div>";
                      $("#resultAppendTarget .list-box").after(pageTemplate);
                    }
                ...
                for (var i = 0; i < data.multiCategorys.length; i++) {
                  ...
                  // LGEGMC-526
                  var pHtml = filter.createProductItem(
                    p.productList,
                  ...
                  $(filter.result.querySelector(".promotion-wrapper")).append(
                    $promoTemplate.get(0).outerHTML
                  );
                  bindImgError();
                  runBVStaticPLP($(filter.result));
                  if (typeof renderListingInlineRatingsRU != "undefined")
                ...
                // empty data;
                $(filter.result.querySelector(".no-content"))
                  .show()
                  .siblings()
                  .hide();
              ...
              if (
                ISVIP &&
                $("input[name=vipPriceOnly]").length > 0 &&
                data.obsVipTotalCount > 0 &&
                data.vipPriceOnlyFlag == "Y"
              ) {
                //LGEDE-365 : 20210713 modify
                var $vipOnly = $("input[name=vipPriceOnly]");
                $vipOnly.closest("li").removeAttr("hidden");
                $vipOnly
                  .siblings(".text")
                  .find(".filter-cnt")
                  .text("(" + data.obsVipTotalCount + ")");
              }
              // PJTOBS-32 End
              //GPC Compare list whitespace 반영
              if ($(".result-box").hasClass("compare-list")) {
                console.log(
                  data.productList[0].priceAreaYn,
                  data.productList[0].promotionAreaYn
                );
                if (
                  data.productList[0].priceAreaYn == "Y" &&
                  data.productList[0].promotionAreaYn == "N"
                ) {
                  setTimeout(function () {
                ...
                if (
                  data.productList[0].priceAreaYn == "N" &&
                  data.productList[0].promotionAreaYn == "Y"
                ) {
                  setTimeout(function () {
                    $(".price-area").remove();
      ...
      viewAllPromotionPLP: function (e, isWholeBtn) {
        ...
        var newParam =
          $(filter.el)
            .serialize()
            .replace(/\&filterCategoryId=[A-Z]+[0-9]+/g, "") +
          "&filterCategoryId=" +
          $listMore.data("category-id");
          ...
          ajax.call(
            ...
            function (d) {
              ...
              data.productList = !data.productList
                ? !data.multiCategorys[0]
                  ? data.multiCategorys
                  : data.multiCategorys[0].productList
                : data.productList;
              if (data.productList) {
                //var data =  d.data instanceof Array ? d.data[0] : d.data,
                var html = filter.createProductItem(
                  data.productList,
                  data.productMessages
                );
                ...
                if (thumbnailLoop)
                  $(filter.result).trigger("thumbnailCarousel");
              }
            },
            null,
            "body"
      ...
      addEvent: function () {
        $(filter.el).off("submit").on({
          /* BTOBGLOBAL-434 : 20210414 modify */ submit: filter.loadPLP,
        });
        $(filter.el)
          .off("change")
          .on(
                ...
                if (
                  ...
                  if (ui && ui.handle) {
                    ...
                    if (
                      ...
                      inputArray[idx].value !=
                        inputArray[idx].getAttribute("data-old")
                    ) {
                      var v =
                        idx == 0
                          ? $(this)
                          ...
                          : $(this)
                              ...
                              .text()
                              .replace(/[^0-9.]/g, "");
                      adobeTrackEvent("product-list-filter", {
                        filter_name:
                          inputArray[idx].getAttribute("name") + "_" + v,
                        page_event: { filter_apply: true },
                      });
                    }

                    inputArray[idx].setAttribute(
                      "data-old",
                      inputArray[idx].value
                    );

                    if (
                      $(this).siblings().find(".min-box input").val() == "" &&
                      $(this).siblings().find(".max-box input").val() == ""
                    ) {
                      $(
                        ".GPC0007 .apply-filters span[data-filter-value=" +
                      ...
                      var relVal =
                        $(this)
                          ...
                          .text() +
                        " : " +
                        $(this).siblings().find(".min-box span").text() +
                        "-" +
                        $(this).siblings().find(".max-box span").text();
                      ...
                      if (
                        ...
                        var $filter_icon =
                          ...
                          removeTxt +
                          "</span></a></span>";
                        $(".GPC0007 .apply-filters .link-text").before(
                          $filter_icon
                        );
                        $(
                          ".GPC0007 .apply-filters span[data-filter-value=" +
                    ...
                    if ($(".GPC0007 .apply-filters span").length != 0) {
                      $(".GPC0007 .apply-filters a.link-text").addClass(
                        "active"
                      );
                    } else {
                      $(".GPC0007 .apply-filters a.link-text").removeClass(
                    ...
                    if (
                      ...
                      if (
                        ...
                        var $filter_icon =
                          ...
                          removeTxt +
                          "</span></a></span>";
                        $(".GPC0026 .apply-filters .link-text").before(
                          $filter_icon
                        );
                        $(
                          ".GPC0026 .apply-filters span[data-filter-value=" +
                    ...
                    if ($(".GPC0026 .apply-filters span").length != 0) {
                      $(".GPC0026 .apply-filters a.link-text").addClass(
                        "active"
                      );
                    } else {
                      $(".GPC0026 .apply-filters a.link-text").removeClass(
                    ...
                    var val =
                      $(this).parent().find(".text .name").length > 0
                        ? changeTitleFormat(
                            $(this).parent().find(".text .name").text()
                          )
                        : changeTitleFormat(
                            $(this).parent().find(".text").text()
                          );
                    var relVal2 =
                      $(this).parent().find(".text .filter-cnt").length == 0
                        ? $(this).parent().find(".text").text()
                        : $(this)
                            .parent()
                            .find(".text")
                            .text()
                            .replace(
                              $(this).parent().find(".text .filter-cnt").text(),
                              ""
                            );
                    var checkName = $(this).attr("name");
                    if (!val || val == "") {
                    ...
                    if ($(this).prop("checked")) {
                      ...
                      $(
                        ...
                        e.preventDefault();
                        if (
                          $(this).parent().attr("data-filter-value") !== "Y"
                        ) {
                          $(
                            '.GPC0007 #categoryFilterForm input[value="' +
                              $(this).parent().attr("data-filter-value") +
        ...
        $(filter.el).off("reset").on({
          /* BTOBGLOBAL-434 : 20210414 modify */ reset: filter.reset,
        });
        $(".GPC0007 .apply-filters .link-text")
          .off("click")
          .on("click", function (e) {
            /* BTOBGLOBAL-434 : 20210414 modify */ e.preventDefault();
            /* BTOBGLOBAL-434 : 20210414 modify */
            if (
              COUNTRY_CODE.toLowerCase() == "global" &&
              $(".navigation").hasClass("b2b")
              ...
              $(filter.el)
                .find('.etc-box button[type="reset"]')
                .trigger("click");
            }
            /*// BTOBGLOBAL-434 : 20210414 modify */
          });
        // LGEGMC-526
        $(".GPC0026 .apply-filters .link-text")
          .off("click")
          .on("click", function (e) {
            /* BTOBGLOBAL-434 : 20210414 modify */ e.preventDefault();
            /* BTOBGLOBAL-434 : 20210414 modify */
      ...
      addAriaDescribedby: function () {
        ...
        $(_this.el.list)
          ...
          .each(function () {
            var $target;
            if (
              $(this).find(".model-name a") &&
              !$(this).find(".model-name a").is(":empty")
            ) {
            ...
            if ($target) {
              ...
              $(this)
                .find("a.btn")
                .attr("aria-describedby", "wa_PLP_" + waNumber);
              $(this)
                .find("a.js-compare")
                .attr("aria-describedby", "wa_PLP_" + waNumber)
                .attr("role", "button");
              ...
              click: function (e) {
                ...
                if (
                  ...
                  if ($(".promotion-wrapper .list-more").length > 0) {
                    ...
                    $(".promotion-wrapper .list-more a:visible").trigger(
                      "click",
                      [true]
                    );
                    ...
                    if ($(e.currentTarget).is(".view-all")) {
                      var viewAllMsg =
                        $(".view-all").siblings(".msg-sort").text() ||
                  ...
                  if ($(e.currentTarget).is(".view-all")) {
                    ...
                    $(e.currentTarget).is(".pagination a") ||
                    $(e.currentTarget).is(".pagination button")
                  ) {
                    ...
                    COUNTRY_CODE.toLowerCase() == "global" &&
                    $(".navigation").hasClass("b2b") &&
                    $(e.currentTarget).is(".expander button")
                  ) {
                    pageHistory.setHistory();
                  }
                  /*// BTOBGLOBAL-434 : 20210414 add */
        ...
        $(plp.el.list).on(
          {
            change: function (e) {
              if (
                ...
                if ($(".GPC0007").length > 0) {
                  $(
                    ".GPC0007 .filter-open-floating .float-sort-box .sort-select"
                  )
                    .find('select option[value="' + sort + '"]')
                    .prop("selected", true);
                }
                ...
                if ($(".GPC0026").length > 0) {
                  $(
                    ".GPC0026 .filter-open-floating .float-sort-box .sort-select"
                  )
                    .find('select option[value="' + sort + '"]')
                    .prop("selected", true);
                }
                ...
                var sortMsg =
                  $("#sortBy")
                    .siblings(".msg-sort")
                    .text()
                    .replace(/\*msg\*/g, sortTxt) ||
                  "Results are sorted by " + sortTxt;
  ...
  if ($("#result-box-aria").length > 0) {
    filter2 = {
            ...
            if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
              ...
              $(window).on("load scroll", function () {
                ...
                if (scrollPos >= parseInt(filterLine) && isFilterFloating) {
                  ...
                  if (
                    $(
                      ".GPC0007 #categoryCurFilterForm, .GPC0026 #categoryFilterForm"
                    )
                      .attr("class")
                      .match("open")
                  ) {
              ...
              $(
                ...
                change: function (e) {
                  if (
                    ...
                    $("html, body").animate(
                      {
                        scrollTop: $(plp2.el.scrollTarget)
                          .closest(".component")
                          .offset().top,
                      },
                      500
              ...
              // PJTCUR-3
              $(
                "#result-box-aria .filter-open-floating .float-sort-box .sort-select .sortBy"
              ).on({
                change: function (e) {
                  if (
                    typeof ePrivacyCookies == "undefined" ||
                    ...
                    $(plp2.el.form).submit();
                    $(
                      "#result-box-aria .filter-open-floating .floating-filter"
                    ).removeClass("active");
                    $("#result-box-aria #categoryCurFilterForm").removeClass(
                      "open"
                    );
                    $("#result-box-aria .filter-open-box").css(
                      "display",
                      "block"
                    );
                    $("#result-box-aria .filter-open-floating").removeClass(
                      "no-floating"
                    );
                    $("#result-box-aria .result-box .sort-box").css(
                      "display",
                      "block"
                    );
                    $(".filter-box .float-filter-result").css(
      ...
      init: function () {
        ...
        // 2020.10.06 PJTPLP WhiteSpace
        if ($(".GPC0007").length > 0 || $(".GPC0026").length > 0) {
          filter2.labelUseFlag = $("#whiteSpaceCurFlag").attr(
            "data-label-use-flag"
          );
          filter2.useLabelIcon = $("#whiteSpaceCurFlag").attr(
            "data-use-label-icon"
          );
          ...
          // 2021.03.11 PJTCUR add end
          filter2.loginUseFlag = $("#reviewDefaultData").attr(
            "data-login-use-flag"
          );
          filter2.reviewLoginUrl = $("#reviewDefaultData").attr(
            "data-review-login-url"
          );
        ...
        if (pageHistory.getHistory()) {
          ...
          if (param) {
            ...
            if (page == "viewAll") {
              setTimeout(function () {
                $("#resultCurProductList .result-info a.view-all").trigger(
                  "click"
                );
              }, 300);
            } else if (parseInt(page) > 1) {
              setTimeout(function () {
                $("#resultCurAppendTarget .pagination ul li")
                  .eq(parseInt(page) - 1)
                  .find("button")
                  .trigger("click");
              }, 300);
        ...
        if (
          ...
          if (param.length > 0) {
            ...
            for (var i = 0; i < arr.length; i++) {
              ...
              if (
                typeof $("#categoryCurFilterForm").find(
                  "[name=" + name + "]"
                )[0] != "undefined"
              ) {
                if (
                  $("#categoryCurFilterForm").find("[name=" + name + "]")[0]
                    .type == "checkbox" ||
                  !!$("#categoryFilterForm")
                    .find("[name=" + name + "]")
                    .parents(".slider-wrap").length
                  ...
                  hasURLFilter = false;
                  console.log(
                    $("#categoryCurFilterForm").find("[name=" + name + "]")
                  );
                  if (cnt != 0) {
                    result += "|";
                  }
            ...
            if (type == "checkbox") {
              ...
              if (_this.prop("checked")) {
                ...
                $(
                  ...
                  if ($(this).parent().attr("data-filter-value") !== "Y") {
                    $(
                      ".GPC0007 #categoryCurFilterForm input[value=" +
                        $(this).parent().attr("data-filter-value") +
                        "]"
                    ).click();
                  } else {
                    $(
                      ".GPC0007 #categoryCurFilterForm input[name=" +
                        $(this).parent().attr("data-filter-name") +
                        "]"
                    ).click();
                  }
              ...
              if ($slider.length > 0) {
                var sliderType = $slider.attr("data-value");
                var index = filter2.slider.indexOfValue(
                  filter2.slider.value[$slider.attr("data-value")],
                  _param[1]
                );
                $input.data("index", index);
      ...
      createProductItem: function (productList, productMessages) {
        ...
        for (var i = 0; i < productList.length; i++) {
          ...
          if ($(".GPC0134").length > 0 || $(".GPC0142").length > 0) {
            if (i == 0) {
              $("#whiteSpaceCurData [data-category-id]").attr(
                "data-category-id",
                $("input[name='curationId']").val()
              );
              $("#whiteSpaceCurData [data-tag-content-area-yn]").attr(
                "data-tag-content-area-yn",
                productList[i].tagContentAreaYn
              );
              $("#whiteSpaceCurData [data-model-brand-area-yn]").attr(
                "data-model-brand-area-yn",
                productList[i].modelBrandAreaYn
              );
              $("#whiteSpaceCurData [data-price-area-yn]").attr(
                "data-price-area-yn",
                productList[i].priceAreaYn
              );
              $("#whiteSpaceCurData [data-promotion-area-yn]").attr(
                "data-promotion-area-yn",
                productList[i].promotionAreaYn
              );
              $("#whiteSpaceCurData [data-reivew-area-yn]").attr(
                "data-review-area-yn",
                productList[i].reviewAreaYn
              );
              $("#whiteSpaceCurData [data-energy-fiche-yn]").attr(
                "data-energy-fiche-yn",
                productList[i].energyFicheAreaYn
              );
              $("#whiteSpaceCurData [data-campaign-area-yn]").attr(
                "data-campaign-area-yn",
                productList[i].campaignAreaYn
              );
              $("#whiteSpaceCurData [data-btn-area-yn]").attr(
                "data-btn-area-yn",
                productList[i].btnAreaYn
              );
              $("#whiteSpaceCurData [data-emi-msg-area-yn]").attr(
                "data-emi-msg-area-yn",
                productList[i].emiMsgAreaYn
              );
              $("#whiteSpaceCurData [data-emi-use-flag]").attr(
                "data-emi-use-flag",
                productList[i].emiUseFlag
              );
              $("#whiteSpaceCurData [data-spec-msg-flag-area-yn]").attr(
                "data-spec-msg-flag-area-yn",
                productList[i].specMsgFlagAreaYn
              );
              $("#resultCurAppendTarget").attr(
                "categoryId",
                $("input[name='curationId']").val()
              );
              //PJTLIMITQTY_EXTEND
              $("#whiteSpaceCurData [data-limitSale-area-yn]").attr(
                "data-limitSale-area-yn",
                productList[i].limitSaleAreaYn
              );
              $("#whiteSpaceCurData [data-limitSale-use-flag]").attr(
                "data-limitSale-use-flag",
                productList[i].limitSaleUseFlag
              );
              //LGEFR-640
              $("#whiteSpaceCurData [data-repair-area-yn]").attr(
                "data-repair-area-yn",
                productList[i].repairModelAreaYn
              );
            }
          }
          var p = productList[i],
          ...
          if ($(".GPC0134").length > 0 || $(".GPC0142").length > 0) {
            ...
            var obsConvertSubscriptionMonthlyCost = changeFormatFullPrice(
              p.obsConvertSubscriptionMonthlyCost,
              p.obsConvertSubscriptionMonthlyCostCent
            );
            if (
              p.obsConvertSubscriptionMonthlyCost != null &&
              obsConvertSubscriptionMonthlyCost != "0"
            ) {
              if (currencyPosition != null) {
                obsConvertSubscriptionMonthlyCostArea =
                  currencyPosition == "R"
                    ? obsConvertSubscriptionMonthlyCost + "" + currencySymbol
                    : currencyPosition == "L"
                    ? currencySymbol + "" + obsConvertSubscriptionMonthlyCost
                    : "";
              }
            }
            // LGCOMPL-46 End
          ...
          template = template
            ...
            // LGCOMPL-46 Start
            .replace(
              /\*obsSubscriptionLandingPageUrl\*/g,
              p.obsSubscriptionLandingPageUrl != null
                ? p.obsSubscriptionLandingPageUrl
                : ""
            )
            .replace(
              /\*obsSubscriptionCtaLinkTarget\*/g,
              p.obsSubscriptionCtaLinkTarget != null
                ? p.obsSubscriptionCtaLinkTarget
                : ""
            )
            .replace(
              /\*obsSubscriptionDisclaimer\*/g,
              p.obsSubscriptionDisclaimer != null
                ? p.obsSubscriptionDisclaimer
                : ""
            )
            .replace(
              /\*obsConvertSubscriptionMonthlyCostArea\*/g,
              obsConvertSubscriptionMonthlyCostArea != ""
                ? obsConvertSubscriptionMonthlyCostArea
                : ""
            )
            .replace(
              /\*obsSubscriptionMaxMonth\*/g,
              p.obsSubscriptionMaxMonth != null ? p.obsSubscriptionMaxMonth : ""
            );
          // LGCOMPL-46 End
          ...
          //LGEGMC-3675 START
          if (
            typeof p.pdrCompareUseFlag != "undefined" &&
            null != p.pdrCompareUseFlag
          ) {
            if (p.pdrCompareUseFlag == "N") {
              $template.find("a.js-compare").css("display", "none");
            }
          }
          //LGEGMC-3675 END
          ...
          if (productType == "card") {
            ...
            // LGCOMPL-46 Start
            if (
              p.obsSubscriptionEnableFlag != null &&
              p.obsSubscriptionEnableFlag == "Y"
            ) {
              $template.find("a.subscribe-box").removeClass("d-none");
            }
            // LGCOMPL-46 End
          } else {
            // default type
            ...
            if (p.buyNowUnionStoreBtnFlag == "Y") {
              $template.find(".button a.buyNowUnionBtn").addClass("active");
            } else {
              $template.find(".button a.buyNowUnionBtn").removeClass("active");
            }
            // Find a dealer btn
            if (
              p.findTheDealerFlag == "Y" &&
      ...
      //PJTCURATION-8 Start
      createProductItemCuration: function (productList, productMessages) {
        var html = [];
        for (var i = 0; i < productList.length; i++) {
          var p = productList[i],
            template = $(".GPC0134 .result-sub-aria .GPC0004")
              .find(".contents-template")
              .clone()
              .html();
          var priceValue = "";
          if (
          ...
          if (
            p.rMembershipPrice != null &&
            p.rMembershipPrice != "" &&
            p.rMembershipPrice != "null"
          ) {
            membershipPriceValue =
              nvl(p.rMembershipPrice, "") +
              "." +
              nvl(p.rMembershipPriceCent, "00");
          }
          ...
          var obsMembershipLinkTarget = p.obsMembershipLinkTarget,
            obsMembershipLinkUrl = p.obsMembershipLinkUrl,
            recommendedRetailRriceInfo = $(
              ".GPC0134 .result-sub-aria .GPC0004"
            ).data("recommended-retail-price-info"),
            rPrice = p.rPrice,
            ...
            emiMsgFlag = !!p.obsEmiMsgFlag && p.obsEmiMsgFlag == "Y";

          var priceType,
            tempPrice = "",
            tempPromoPrice = "",
            tempLegal = "";
          (obsMemberShipLinkStartHtml = ""), (obsMemberShipLinkEndHtml = "");
          ...
          // LGCOMPL-46 Start
          var currencyPosition = $(".GPC0134")
            .find("input[name=currencyPosition]")
            .val();
          var currencySymbol = $(".GPC0134")
            .find("input[name=currencySymbol]")
            .val();
          var obsConvertSubscriptionMonthlyCostArea = "";
          var obsConvertSubscriptionMonthlyCost = changeFormatFullPrice(
            p.obsConvertSubscriptionMonthlyCost,
          ...
          template = template
            ...
            .replace(
              /\*externalLinkTarget\*/g,
              externalLinkTarget == "self" ? "_self" : "_blank"
            )
            .replace(
              /\*reStockAlertUrl\*/g,
              p.reStockAlertUrl ? p.reStockAlertUrl : ""
            ...
            .replace(
              /\*rPromoPrice\*\.\*rPromoPriceCent\*/g,
              p.rPromoPrice
                ? changeFormatFullPrice(p.rPromoPrice, p.rPromoPriceCent)
                : "null"
            )
            .replace(
              /\*rPrice\*/g,
              p.rPrice ? changeFormatPrice(p.rPrice) : "null"
            )
            .replace(
              /\*rPromoPrice\*/g,
              p.rPromoPrice ? changeFormatPrice(p.rPromoPrice) : "null"
            )
            .replace(/\*rPriceCent\*/g, p.rPriceCent ? p.rPriceCent : "null")
            .replace(
              /\*rPromoPriceCent\*/g,
              p.rPromoPriceCent ? p.rPromoPriceCent : "null"
            )
            /* in-house review rating add */
            .replace(/\*sRating2\*/g, p.reviewRatingStar2)
            .replace(/\*pCount\*/g, p.reviewRating)
            .replace(/\*ratingPercent\*/g, p.reviewRatingPercent)
            /* // in-house review rating add */
            .replace(/\*siblingType\*/g, p.siblingType)
            .replace(/\*discountedRate\*/g, p.discountedRate)
            .replace(/\*retailerPricingText\*/g, p.retailerPricingText)
            ...
            .replace(/\*emiPopUrl\*/g, p.emiPopUrl || "")
            .replace(
              /\*afterPayInstallMent\*/g,
              p.obsEmiMsgFlag == "Y" &&
                p.emiMsg != null &&
          ...
          // LGCOMPL-46 End
          var $obj = $(".GPC0134 .result-sub-aria .GPC0004");
          var pdfDownloadFile = $obj
            .find('input[name="pdfDownloadFile"]')
            .val();
          var productFicheDownload = $obj
            .find('input[name="productFicheDownload"]')
            .val();
          var rsProductFicheDownload = $obj
            .find('input[name="rsProductFicheDownload"]')
            .val();
          var rsUseFlag = p.rsUseFlag;
          if (rsUseFlag == "Y") {
          ...
          if (
            ...
            var productFicheDownload = $obj
              .find('input[name="productNewFicheDownload"]')
              .val();
          } else if (docTypeCodeFlag == "O") {
            var productFicheDownload = $obj
              .find('input[name="productOldFicheDownload"]')
              .val();
          }
          //LGEGMC-5315
          ...
          for (var i1 = 0; i1 < $keyBlocks.length; i1++) {
            ...
            if (
              p.obsLimitSale != "Y" &&
              (p.productTag1ColorFlag == "Y" || p.productTag2ColorFlag == "Y")
            ) {
              switch ($template.find(".tag-content span").length) {
                case 1:
                  if (p.productTag1 != null && p.productTag1ColorFlag == "Y") {
                    $($template.find(".tag-content span")[0]).addClass(
                      "flag-gold"
                    );
                  }
                  if (p.productTag2 != null && p.productTag2ColorFlag == "Y") {
                ...
                case 2:
                  ...
                  if (p.productTag2 != null && p.productTag2ColorFlag == "Y") {
                    $($template.find(".tag-content span")[1]).addClass(
                      "flag-gold"
                    );
                  }
                  break;
            ...
            if (p.vipPriceFlag == "Y") {
              ...
              if (p.obsLimitSale == "Y" && p.limitSaleUseFlag == "Y") {
                ...
                if (countryCode == "it") {
                  ...
                  $template
                    .find(".price-vip")
                    .text(productMessages.limitSaleTitle);
                }
              } else {
                $template.find(".price-vip").text("");
              }
          ...
          if (
            (OBS_LOGIN_FLAG == "N" || OBS_LOGIN_FLAG == "T") &&
            p.buyNowUseFlag == "N" &&
            p.resellerBtnFlag == "N" &&
            p.productSupportFlag == "N"
          ) {
            // change color (No red button)
            if ($template.find(".button a.add-to-cart").length > 0)
              $template
                .find(".button a.add-to-cart")
                .removeClass("btn-primary")
                .addClass("btn-outline-secondary");
            if ($template.find(".button a.where-to-buy").length > 0)
              $template
                .find(".button a.where-to-buy")
          ...
          // Price
          if (p.modelStatusCode == "ACTIVE") {
            if (!!rPromoPriceCent) {
              tempPrice = changeFormatFullPriceSync(
                rPromoPrice,
                rPromoPriceCent
              );
              tempPromoPrice = changeFormatFullPriceSync(rPrice, rPriceCent);
              tempLegal = p.discountMsg;
            } else if (!!rPromoPrice) {
          ...
          if (bizType == "B2B" && obsLoginFlag == "Y" && !!rPrice) {
            $template.find(".price-area > div.msrp").remove();
          }
          if (!!p.membershipDisplayFlag && p.membershipDisplayFlag != "Y") {
            $template.find(".member-text").remove();
          }
          if (!!!rPrice || !!!rPromoPrice) {
            $template.find(".recommended-retail-price").remove();
          }
          if (p.modelStatusCode != "ACTIVE") {
            $template.find(".purchase-price, .product-price").remove();
          }
          if (!!p.retailerPricingFlag && p.retailerPricingFlag != "Y") {
            $template.find(".price-area > div.text").remove();
          }
          if (
            recommendedRetailRriceInfo != "Y" ||
            (recommendedRetailRriceInfo != "Y" && !!!rPrice && !!!rPromoPrice)
          ) {
            $template.find(".recommended-retail-price").remove();
          }
          if (p.afterPay <= 0 || p.obsInventoryFlag != "Y") {
            $template
              .find(".price-vip-Installment > a.afterpay-installment")
              .remove();
          }

          // Setting
          if (p.modelStatusCode == "ACTIVE") {
            $template.find(".purchase-price .number").text(tempPrice);
            $template.find(".product-price .number").text(tempPromoPrice);
            $template.find(".product-price .legal").text(tempLegal);
          }
          if (
            !!p.limitSaleUseFlag &&
            p.limitSaleUseFlag == "Y" &&
            ...
            var countryCode = $("[data-countrycode]").attr("data-countrycode");
            if (countryCode == "it") {
              $template.find(".price-vip-Installment .price-vip").text("");
            } else {
              $template
        ...
        ajax.call(
          ...
          function (d) {
            ...
            if (d.status == "success" && data) {
              // reset no-contents area
              if (
                $(filter2.result.querySelector(".no-content")).is(":visible")
              ) {
              ...
              if (
                data.promotionTotalCount != null &&
                filter2.el.querySelector(
                  "input[type=checkbox][name=promotionsOnly]"
                ) != null &&
                data.promotionTotalCount == 0
              ) {
                var promotionsOnlySelector1 = filter2.el.querySelector(
                  "input[type=checkbox][name=promotionsOnly]"
                );
                promotionsOnlySelector1.setAttribute("disabled", "disabled");
                promotionsOnlySelector1.parentNode.classList.add("disabled");
                ...
                var promotionsOnlySelector2 = filter2.el.querySelector(
                  "input[type=checkbox][name=promotionsOnly]"
                );
                promotionsOnlySelector2.removeAttribute("disabled");
                promotionsOnlySelector2.parentNode.classList.remove("disabled");
              ...
              if (
                ...
                var obsOnlySelector1 = filter2.el.querySelector(
                  "input[type=checkbox][name=obsOnly]"
                );
                obsOnlySelector1.setAttribute("disabled", "disabled");
                obsOnlySelector1.parentNode.classList.add("disabled");
                ...
                var obsOnlySelector2 = filter2.el.querySelector(
                  "input[type=checkbox][name=obsOnly]"
                );
                obsOnlySelector2.removeAttribute("disabled");
                obsOnlySelector2.parentNode.classList.remove("disabled");
              ...
              if (
                ...
                var bundlesOnlySelector1 = filter2.el.querySelector(
                  "input[type=checkbox][name=bundlesOnly]"
                );
                bundlesOnlySelector1.setAttribute("disabled", "disabled");
                bundlesOnlySelector1.parentNode.classList.add("disabled");
                ...
                var bundlesOnlySelector2 = filter2.el.querySelector(
                  "input[type=checkbox][name=bundlesOnly]"
                );
                bundlesOnlySelector2.removeAttribute("disabled");
                bundlesOnlySelector2.parentNode.classList.remove("disabled");
              ...
              //if(data.productList.length > 0 || data.multiCategorys.length == 1) {
              if (
                $(filter2.result.querySelector(".promotion-wrapper")).length ==
                  0 &&
                (data.productList.length > 0 || data.multiCategorys.length == 1)
              ) {
                // normal product list & single promotion list
                ...
                // make markup
                //console.log(data);
                html = filter2.createProductItem(
                  data.productList,
                  d.data[0].productMessages
                );
                ...
                if (isExpander) {
                  ...
                  var pageVisible = pageInfo && pageInfo.view == "Y";
                  if (pageVisible && pageInfo.page < pageInfo.pageCount) {
                    filter2.result.querySelector(".expander button").value =
                      pageInfo.page + 1;
                  ...
                  if (typeof renderListingInlineRatingsRU != "undefined")
                    renderListingInlineRatingsRU(getProductsNameRU());
                  if (data.totalCount) {
                    $(".result-curation-head .result-number").text(
                      data.totalCount
                    );
                    $("#resultCurProductList .result-number").text(
                      data.totalCount
                    );
                  ...
                  /* 20191111 : LGEUS-11779 add */
                  if (
                    $("#categoryCurFilterForm input[type=checkbox]:checked")
                      .length == 0 &&
                    $('#categoryCurFilterForm .slider-wrap input[value!=""]')
                      .length == 0
                  ) {
                  ...
                  //PJTCURATION-8_2 Start
                  var fastViewType = $("input[name='resultViewType']").val();
                  if (fastViewType == "S") {
                    var subArea = $(".GPC0134 .result-sub-aria .GPC0004");
                    subArea.hide();
                    var oriTxt = $("input[name='fastBtnOrgin']").val();
                    var fastCount = data.totalCount;
                    if (
                      parseInt(fastCount) > 0 &&
                      typeof oriTxt !== "undefined"
                    ) {
                      /* LGETW-651 - oriTxt undefined check */
                      $(".btn-fast").html(oriTxt.replace("%", fastCount));
                      $(".fast-move").show();
                    } else {
                      $(".fast-move").hide();
                    }
                    $(".btn-fast").on("click", function () {
                      $(".step-box").hide();
                      $(".result-curation-box").show();
                      if (subArea.find("li.item").length > 0) {
                        subArea.find(".slick-slider").slick("setPosition");
                        subArea.show();
                      }
                      if ($(".result-box-prev").length) {
                        $(".result-box-prev").show();
                      }
                      var resultCount = $(
                        ".result-info .total1 .result-number"
                      ).text();
                      if (parseInt(resultCount) > 0) {
                        var plpBox = $(".product-list-box");
                        if (
                          $(plpBox.find(".no-content")).css("display") != "none"
                        ) {
                          if (parseInt(resultCount) > 16) {
                            $(plpBox.find(".list-box")).show();
                            $(plpBox.find(".no-content"))
                              .hide()
                              .siblings()
                              .show();
                          } else {
                            $(plpBox.find(".no-content")).hide();
                            $(plpBox.find(".list-box")).show();
                          }
                        }
                      } else {
                        var resultCount2 = $(
                          ".result-text .total1 .result-number"
                        );
                        if (resultCount != resultCount2.text()) {
                          resultCount2.text(resultCount);
                        }
                      }
                    });
                  }
                  ...
                  if (
                    Number(
                      $(
                        "#resultCurProductList .total2 .result-number-total"
                      ).text()
                    ) <
                    Number(
                      $("#resultCurProductList .total2 .result-number").text()
                    )
                  ) {
                    $(
                      "#resultCurProductList .total2 .result-number-total"
                  ...
                  if (pageInfo && pageInfo.view == "Y") {
                    if (filter2.result.querySelector(".pagination")) {
                      ...
                      for (
                        ...
                        /* LGEGMC-754 : 20201123 modify */
                        var _pageItem = filter2.pageTemplate
                          ? filter2.pageTemplate
                          : '<li><button type="button" value=""></button></li>';
                        /*// LGEGMC-754 : 20201123 modify */
                        _pageItem = $(_pageItem).get(0);
                        var _pageButton = _pageItem.querySelector("button");
                      ...
                      $(filter2.result.querySelector(".pagination ul")).html(
                        pageMarkup
                      );

                      filter2.result.querySelector(
                        ".pagination .prev"
                      ).disabled = !pageInfo.leftPage;
                      filter2.result.querySelector(
                        ".pagination .next"
                      ).disabled = !pageInfo.rightPage;
                      filter2.result.querySelector(".pagination .prev").value =
                        pageInfo.page - 1;
                      filter2.result.querySelector(".pagination .next").value =
                        pageInfo.page + 1;
                      $(filter2.result.querySelector(".pagination")).show();
                    } else if (
                      filter2.result.querySelectorAll(".btn-area").length > 0
                    ) {
                      filter2.result.querySelector(".btn-area .prev").disabled =
                        !pageInfo.leftPage;
                      filter2.result.querySelector(".btn-area .next").disabled =
                        !pageInfo.rightPage;
                    ...
                    if (
                      pageInfo.loopStart != pageInfo.loopEnd &&
                      !!!filter2.result.querySelector(".expander")
                    ) {
                      var pageTemplate = "";
                      pageTemplate += '<div class="expander">';
                      pageTemplate +=
                    ...
                    if (fastViewType == "S") {
                      if (data.totalCount > 16) {
                        $(filter2.result.querySelector(".list-box")).show();
                        $(filter2.result.querySelector(".no-content"))
                          .hide()
                          .siblings()
                          .show();
                      } else if (data.totalCount > 0) {
                        $(filter2.result.querySelector(".no-content")).hide();
                        $(filter2.result.querySelector(".list-box")).show();
                      }
                    }
                    /* PJTCURATION-8_2 add */
                ...
                for (var i = 0; i < data.multiCategorys.length; i++) {
                  ...
                  // LGEGMC-526
                  var pHtml = filter2.createProductItem(
                    p.productList,
                    data.productMessages
                  );
                  var productListHtml = "";
                  ...
                  $(filter2.result.querySelector(".promotion-wrapper")).append(
                    $promoTemplate.get(0).outerHTML
                  );
                  bindImgError();
                  runBVStaticPLP($(filter2.result));
                ...
                if (
                  $(".GPC0134").length > 0 &&
                  $("input[name='resultViewType']").val() == "S"
                ) {
                  $(".GPC0134 .result-sub-aria .GPC0004").hide();
                  $(".fast-move").hide();
                }
                //PJTCURATION-8_2 End
              }
              ...
              /* PJTCURATION-8 Start */
              if (
                data.exampleProductList &&
                data.exampleProductList != null &&
                data.exampleProductList.length > 0
              ) {
                var $target = $(".GPC0134 .result-sub-aria .GPC0004");
                html = filter2.createProductItemCuration(
                  data.exampleProductList,
                  d.data[0].productMessages
                );
                ...
                if (
                  data.siblingAjaxUrl &&
                  data.siblingAjaxUrl != null &&
                  $target.attr("data-model-url") == ""
                ) {
                  $target.attr("data-model-url", data.siblingAjaxUrl);
                }
                ...
                var opt4 = {
                  ...
                  nextArrow: carouselOptions.bigAngleNext,
                };
                $target.find(".unit-list").slick(opt4);
                $target.find(".list-contents-wrap").addClass("initialized");
                bindImgError();
                runBVStaticPLP($target.find(".list-contents-wrap"));
                setTimeout(function () {
                  $target.find(".rating[data-modelid]").each(function () {
                    $(this)
                      .find("a.bv-rating-stars-container")
                      .attr("data-link-area", "selective_offering-product_list")
                      .attr("data-link-name", $(this).data("modelname"));
                  });
                }, 3000);
                ...
                if (!!$target.find(".rating[data-bv-show=inline_rating]")) {
                  setTimeout(function () {
                    $target
                      .find(".rating[data-bv-show=inline_rating]")
                      .each(function () {
                        $(this)
                          .find("a")
                          .attr(
                            "data-link-area",
                            "selective_offering-product_list"
                          )
                          .attr("data-link-name", $(this).data("modelname"));
                      });
                  }, 3000);
                ...
                if (typeof renderListingInlineRatingsRU != "undefined")
                  renderListingInlineRatingsRU(getProductsNameRU());
                var resultViewType = $("input[name='resultViewType']").val();
                if (resultViewType != "S") {
                  $target.show();
                } else {
              ...
              if (data.productList.length > 0) {
                if (data.productList[0].repairModelAreaYn != "Y") {
                  $(".repairability-index").remove();
                }
              }
              ...
              /* LGETW-651 start */
              if (
                !!data.obsInstallmentPriceDisableLink &&
                data.obsInstallmentPriceDisableLink == "Y"
              ) {
                $("#resultCurProductList")
                  .find(".price-installment")
                  .addClass("no-link")
                  .attr("tabindex", "-1");
              }
              /* LGETW-651 end */
        ...
        if (url && url != null) {
          ajax.call(
            ...
            function (d) {
              ...
              if (data.productList) {
                //var data =  d.data instanceof Array ? d.data[0] : d.data,
                var html = filter2.createProductItem(
                  data.productList,
                  data.productMessages
                );
                ...
                if (thumbnailLoop)
                  $(filter2.result).trigger("thumbnailCarousel");
              }
            },
            null,
            "body"
            ...
            change: function (event, ui) {
              ...
              if (
                ...
                if (ui && ui.handle) {
                  ...
                  var name = changeTitleFormat(
                    $(this).closest(".option-box").find(".title strong").text()
                  );
                  var val =
                    $(this).parent().find(".text .name").length > 0
                      ? changeTitleFormat(
  ...
  var fileDownload = function ($link) {
    ...
    if (flag == "Y") {
      ...
      var input =
        '<input type="hidden" id="DOC_ID" name="DOC_ID" value="' + doc + '" />';
      input +=
        '<input type="hidden" id="ORIGINAL_NAME_b1_a1" name="ORIGINAL_NAME_b1_a1" value="' +
        original +
        '" />';
      input +=
      ...
      input +=
        '<input type="hidden" id="GSRI_DOC" name="GSRI_DOC" value="GSRI" />';
      input +=
        '<input type="hidden" id="SPEC_DOWNLOAD" name="SPEC_DOWNLOAD" value="N" />';

      /*LGEGMC-1035 start*/
      var openlocale =
        "|AT|BE_FR|BG|CH_DE|CH_FR|CZ|DE|DK|EE|ES|FI|FR|GR|HR|HU|IT|LT|LV|NL|NO|PL|PT|RO|RS|SE|SK|UK|";
      var _target = "_self";
      if (
        (original.toLowerCase().indexOf(".pdf") > -1 ||
          original.toLowerCase().indexOf(".bmp") > -1 ||
          original.toLowerCase().indexOf(".jpg") > -1 ||
          original.toLowerCase().indexOf(".jpeg") > -1 ||
          original.toLowerCase().indexOf(".png") > -1) &&
        openlocale.indexOf("|" + COUNTRY_CODE.toUpperCase() + "|") > -1
      ) {
        _target = "_blank";
      }
  ...
  $(document).on(
    "click",
    ".GPC0007 .file-list a.link-text,.compare-product .file-list a.link-text, .GPC0026 .file-list a.link-text",
    function (e) {
      e.preventDefault();
      fileDownload($(this));
    }
  ...
  /* LGEPL-538 Start */
  $(this).on("mouseover", ".member-text a", function () {
    $(this).find("span").addClass("member-link");
  });
  $(this).on("mouseleave", ".member-text a", function () {
    $(this).find("span").removeClass("member-link");
  });
  /* LGEPL-538 End */

  /* LGEBE-4 : 20200605 add */
  var pdp_where = {
    wtbOfflineCategoryIds: document.querySelectorAll(
      "input[name=wtbOfflineCategoryIds"
    ),
    ...
    init: function () {
      var self = this;
      if (
        $(self.wtbOfflineCategoryIds).val() != undefined &&
        $(self.wtbOfflineCategoryIds).val() != "" &&
        $(self.wtbOfflineCategoryIds).val() != "component-WtbOfflineCategory"
      ) {
        ...
        if (self.aflag) {
          for (var i = 0; i < $aTag.length; i++) {
            ...
            var url =
              aTagHref.substring(0, aTagHref.indexOf("#pdp_where")) +
              "#pdpOfflieWhere";
